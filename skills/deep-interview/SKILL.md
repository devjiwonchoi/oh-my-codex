...
<Purpose>
Deep Interview is an intent-first Socratic clarification loop before planning or implementation. It turns vague ideas into execution-ready specifications by asking targeted questions about why the user wants a change, how far it should go, what should stay out of scope, and what NOMX may decide without confirmation.
</Purpose>
...
- `nomx explore` is deprecated. Use normal repository inspection tools/subagents for simple read-only brownfield fact gathering; use `nomx sparkshell` only for explicit shell-native read-only evidence, and keep ambiguous or non-shell-only investigation on the richer normal path.
- Always run a preflight context intake before the first interview question
- For brownfield work, preflight must include doc/context grounding before user-facing questions: inspect applicable `AGENTS.md` files, README/getting-started docs, relevant `docs/` contracts/plans/ADRs, existing `.nomx/context/` snapshots, and any project-local glossary/context files such as `CONTEXT.md` or `CONTEXT-MAP.md` when present.
...
- Persist mode state for resume safety with CLI-first state commands (`nomx state write/read --input '<json>' --json`); use `state_write` / `state_read` only when explicit MCP compatibility is enabled
</Execution_Policy>
...
2. Attempt to load the latest relevant context snapshot from `.nomx/context/{slug}-*.md`.
...
5. For brownfield tasks, inspect the applicable documentation/rule surface before the first user-facing round. Prefer exact, nearby sources over broad scans:
   ...
   - existing `.nomx/context/` snapshots, `.nomx/specs/`, and planning artifacts relevant to the slug
   - project-local glossary/context files such as `CONTEXT.md`, `CONTEXT-MAP.md`, or context-specific docs when they exist
6. Save snapshot to `.nomx/context/{slug}-{timestamp}.md` (UTC `YYYYMMDDTHHMMSSZ`) and reference it in mode state.
...
3. Initialize state via `nomx state write --input '{"mode":"deep-interview","active":true}' --json`:
...
```json
{
  ...
  "state": {
    ...
    "context_snapshot_path": ".nomx/context/<slug>-<timestamp>.md"
  }
}
```
...
### 2e) Persist state
Append round result and updated scores via `nomx state write --input '<json>' --json`; use `state_write` only when explicit MCP compatibility is enabled.
...
1. Write interview transcript summary to:
   - `.nomx/interviews/{slug}-{timestamp}.md`
     (kept for ralph PRD compatibility)
2. Write execution-ready spec to:
   - `.nomx/specs/deep-interview-{slug}.md`
...
- Decision Boundaries (what NOMX may decide without confirmation)
- Constraints
...
- Save transcript/spec artifacts under `.nomx/interviews/` and `.nomx/specs/`
</Tool_Usage>
...
<Final_Checklist>
- [ ] Preflight context snapshot exists under `.nomx/context/{slug}-{timestamp}.md`
...
- [ ] Transcript written to `.nomx/interviews/{slug}-{timestamp}.md`
- [ ] Spec written to `.nomx/specs/deep-interview-{slug}.md`
...
1. Repository-local `.nomx/config.toml`
2. Repository-root `omx.toml`
3. User-global `~/.nomx/config.toml`

This section is currently a deep-interview-specific runtime override surface, not a general replacement for Codex `config.toml` or `.nomx-config.json` model/env routing.
...
If interrupted, rerun `$deep-interview`. Resume from persisted mode state via `nomx state read --input '{"mode":"deep-interview"}' --json`.
