...
<Purpose>
Ultrawork is a parallel execution engine for high-throughput task completion. It provides native-subagent parallelism, context discipline, and smart delegation guidance, but not durable goal tracking, Ralph's legacy persistence loop, architect sign-off, or long-running completion guarantees.
</Purpose>
...
Use the CLI-first state surface (`nomx state ... --json`) for ultrawork lifecycle state. If explicit MCP compatibility tools are already available, equivalent `nomx_state` calls are optional compatibility, not the default.
...
- Use additional native Codex subagents when independent bounded lanes materially improve throughput.
