...
`ultragoal` turns a brief into repo-native artifacts and then drives a Codex goal safely through goal tools. New plans default to a stable pointer-style aggregate Codex goal for the whole durable plan in `.nomx/ultragoal/goals.json`, including later accepted/appended stories under the original brief constraints, while NOMX tracks G001/G002 story progress in the ledger. Ultragoal does not call Codex `/goal clear`; before multiple sequential ultragoal runs in one Codex session/thread, manually run `/goal clear` in the Codex UI so the previous completed aggregate goal does not block or confuse the next `create_goal`.

- `.nomx/ultragoal/brief.md`
- `.nomx/ultragoal/goals.json`
- `.nomx/ultragoal/ledger.jsonl` (checkpoint and structured steering audit events)
...
- **Standalone `$ultragoal` activation**: ensure `.nomx/state[/sessions/<session>]/ultragoal-state.json` exists with `mode:"ultragoal"`, `active:true`, and a non-empty `current_phase` such as `planning` before or while goals are created. This state is a lightweight HUD/runtime declaration; `.nomx/ultragoal/goals.json` and `ledger.jsonl` remain the durable goal source of truth.
...
2. Inspect `.nomx/ultragoal/goals.json` and refine if needed.
...
3. Call `get_goal`.
4. If no active Codex goal exists, call `create_goal` with the printed payload. In aggregate mode, if the same aggregate Codex objective is already active, continue the current NOMX story without creating a new Codex goal.
5. Complete the current NOMX story only.
...
- Do not edit the aggregate Codex objective, original brief constraints, quality gates, or completion status. The aggregate objective is a stable pointer to `.nomx/ultragoal/goals.json` and `.nomx/ultragoal/ledger.jsonl`, not an enumeration of initial goal ids.
- Do not hard-delete goals, auto-complete work, weaken verification, or silently mutate `.nomx/ultragoal`.
- Accepted and rejected attempts append structured audit entries to `.nomx/ultragoal/ledger.jsonl`.
