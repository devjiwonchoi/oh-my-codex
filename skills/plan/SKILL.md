...
- `nomx explore` is deprecated. Use normal repository inspection tools/subagents for simple read-only repository lookups during planning; use `nomx sparkshell` only for explicit shell-native read-only evidence, and keep prompt-heavy or ambiguous planning work on the richer normal path.
...
2. **Ask one focused question** using the surface-appropriate structured question path for preferences, scope, and constraints: in attached-tmux OMX runtime use `nomx question`; outside tmux use native structured input when available; use plain text only as a last fallback
