...
const NOMX_PLUGIN_HOOK_LAUNCHER_CONTRACT_MARKER = 'nomx-plugin-hook-launcher:v1';
...
function resolveLaunchClaimPath(payload, launchId) {
  ...
  const stateRoot = typeof process.env.NOMX_ROOT === 'string' && process.env.NOMX_ROOT.trim()
    ? process.env.NOMX_ROOT.trim()
    : join(cwd, '.nomx');
...
function isOmxLauncherSession(input, payload) {
  const launchId = process.env.NOMX_CODEX_LAUNCH_ID?.trim();
  const entryPath = process.env.NOMX_ENTRY_PATH?.trim();
...
function stopFallbackOutput(stopReason, detail) {
  const reason = 'NOMX plugin Stop hook launcher failed before valid native Stop JSON could be produced. Continue once, preserve runtime state, inspect hook launcher diagnostics, and retry.';
  return {
    decision: 'block',
    reason,
    stopReason,
...
function writeOversizedStopFallback(stopReason, detail) {
  const reason =
    'NOMX plugin Stop rejected oversized stdin before launcher delegation; continue once with a smaller valid Stop JSON response.';
...
function readPinnedLauncher() {
  const launcherPath = join(hookDir, 'nomx-command.json');
  try {
...
function readConfiguredLauncher() {
  if (process.env.NOMX_NATIVE_HOOK_COMMAND) {
    return { command: process.env.NOMX_NATIVE_HOOK_COMMAND, argsPrefix: [] };
  }
...
function listAuthoritativeStateBaseDirs(cwd) {
  if (process.env.NOMX_TEAM_STATE_ROOT?.trim()) return [process.env.NOMX_TEAM_STATE_ROOT.trim()];
  if (process.env.NOMX_ROOT?.trim()) return [join(process.env.NOMX_ROOT.trim(), '.nomx', 'state')];
  if (process.env.NOMX_STATE_ROOT?.trim()) return [join(process.env.NOMX_STATE_ROOT.trim(), '.nomx', 'state')];
...
function readCurrentSessionId(stateBaseDirs, cwd) {
  ...
  const envSessionId = process.env.NOMX_SESSION_ID || process.env.CODEX_SESSION_ID;
...
async function main() {
  ...
  child.on('close', (code, signal) => {
    ...
    if (isCompact) {
      if (childSpawnError) {
        console.error(`[nomx] failed to launch ${command} codex-native-hook: ${childSpawnError.message}`);
      } else if (signal) {
        console.error(`[nomx] codex-native-hook terminated by ${signal}`);
      } else if (code && code !== 0) {
        console.error(`[nomx] codex-native-hook exited with code ${code}`);
      }
      process.exitCode = 0;
      return;
    }
...
main().catch((error) => {
  console.error(`[nomx] plugin hook launcher failed: ${error instanceof Error ? error.message : String(error)}`);
  process.exitCode = 1;
});
