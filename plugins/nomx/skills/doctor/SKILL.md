---
name: doctor
description: Diagnose and fix nomx installation issues
---
...
NOMX installs skills to `${CODEX_HOME:-~/.codex}/skills/` — this is the path current Codex CLI natively loads as its skill root.

`~/.agents/skills/` is a **historical legacy path** from an older Codex CLI release, before Codex settled on `~/.codex` as its home directory. Current Codex CLI and NOMX no longer write there.

**In a mixed NOMX + plain Codex environment:**
...
- **Interop rule**: NOMX writes only to the canonical path; archive or remove `~/.agents/skills/` once you have confirmed `${CODEX_HOME:-~/.codex}/skills/` is your active root

## Task: Run Installation Diagnostics

You are the NOMX Doctor - diagnose and fix installation issues.

### Step 1: Check Plugin Version

Official Codex plugin caches are marketplace- and version-scoped, for example `${CODEX_HOME:-~/.codex}/plugins/cache/$MARKETPLACE_NAME/nomx/$VERSION/`. Local installs may use `local` as the version identifier.

```bash
# Get installed plugin cache versions across marketplaces.
# Cache shape: $PLUGIN_CACHE_ROOT/$MARKETPLACE_NAME/nomx/$PLUGIN_VERSION/
PLUGIN_CACHE_ROOT="${CODEX_HOME:-$HOME/.codex}/plugins/cache"
CACHE_ENTRIES=$(find "$PLUGIN_CACHE_ROOT" -path "*/nomx/*" -mindepth 3 -maxdepth 3 -type d 2>/dev/null)
...
# Get latest from npm
LATEST=$(npm view nomx version 2>/dev/null)
echo "Latest npm: $LATEST"
```

**Diagnosis**:
- If no cache entry exists: INFO - plugin marketplace artifact not cached; this may be normal when NOMX was installed only through npm/setup
...
- Remember: plugin install/discovery is not a replacement for `npm install -g nomx` plus `nomx setup`; the packaged plugin carries plugin-scoped companion metadata for optional MCP compatibility servers and apps, with first-party MCP disabled by default, while native/runtime hooks and the rest of NOMX runtime wiring stay setup-owned
...
# Check for NOMX marker
grep -q "nomx Multi-Agent System" ~/.codex/AGENTS.md 2>/dev/null && echo "Has NOMX config" || echo "Missing NOMX config"
```

**Diagnosis**:
- If missing: CRITICAL - AGENTS.md not configured
- If missing NOMX marker: WARN - outdated AGENTS.md
...
PLUGIN_CACHE_ROOT="${CODEX_HOME:-$HOME/.codex}/plugins/cache"
find "$PLUGIN_CACHE_ROOT" -path "*/nomx/*" -mindepth 3 -maxdepth 3 -type d 2>/dev/null \
  | while IFS= read -r VERSION_DIR; do
      MARKETPLACE_NAME=$(basename "$(dirname "$(dirname "$VERSION_DIR")")")
      PLUGIN_VERSION=$(basename "$VERSION_DIR")
...
**Diagnosis**:
- If `~/.codex/agents/` exists with nomx-related files: WARN - legacy generated agents or hand-installed role files. The Codex plugin can package reusable workflows plus plugin-scoped companion metadata for optional MCP/apps; legacy setup installs native agents, while plugin setup archives stale legacy native-agent files and keeps config/hooks current.
- If `~/.codex/commands/` exists with nomx-related files: WARN - legacy command files from older installs. Current NOMX uses skills/workflows plus setup-managed native surfaces.
- If `${CODEX_HOME:-~/.codex}/skills/` exists with NOMX skills: OK - canonical current user skill root
...
- Any nomx-related `.md` files in skills/
...
PLUGIN_CACHE_ROOT="${CODEX_HOME:-$HOME/.codex}/plugins/cache"
find "$PLUGIN_CACHE_ROOT" -path "*/nomx" -type d -prune -exec rm -rf {} +
...
PLUGIN_CACHE_ROOT="${CODEX_HOME:-$HOME/.codex}/plugins/cache"
PLUGIN_CACHE_DIR="$PLUGIN_CACHE_ROOT/$MARKETPLACE_NAME/nomx"
...
```
Copy `templates/AGENTS.md` from the installed nomx package to `~/.codex/AGENTS.md`.
```
...
**Note**: Only remove if these contain nomx-related files. If user has custom agents/commands/skills, warn them and ask before removing.
