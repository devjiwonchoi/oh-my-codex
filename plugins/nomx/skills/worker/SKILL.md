---
name: worker
description: Team worker protocol (ACK, mailbox, task lifecycle) for tmux-based NOMX teams
---

# Worker Skill

This skill is for a Codex session that was started as an NOMX Team worker (a tmux pane spawned by `$team`).

## Identity

You MUST be running with `NOMX_TEAM_WORKER` set. It looks like:
