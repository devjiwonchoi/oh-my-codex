...
`$team` is the tmux-based parallel execution mode for NOMX. It starts real worker Codex and/or Claude CLI sessions in split panes and coordinates them through `.nomx/state/team/...` files plus CLI team interop (`nomx team api ...`) and state files.

This skill is operationally sensitive. Treat it as an operator workflow, not a generic prompt pattern. In Codex App or plain outside-tmux sessions, do not present `$team` / `nomx team` as directly available; launch NOMX CLI from shell first, or stay on the nearest app-safe surface until the user explicitly wants the tmux runtime.
...
1. Invoke NOMX runtime directly with `nomx team ...`
...
ATEM fit: treat this as agile teamwork support for transition/action/interpersonal moments around boundaries, not as a heavyweight process model. Do not copy provider-specific plugin implementations; keep the protocol in NOMX/Codex prompts, inboxes, state, and tests.

### Team + Ultragoal bridge

Use `$ultragoal` for durable leader-owned goal/ledger tracking and `$team` for parallel execution lanes. When Team is launched with an active `.nomx/ultragoal/goals.json`, worker inboxes/status may include leader-owned Ultragoal context: `.nomx/ultragoal/goals.json`, `.nomx/ultragoal/ledger.jsonl`, the active goal id, Codex goal mode, and the `fresh_leader_get_goal_required` checkpoint policy.

Workers provide task status and verification evidence only. They do not own Ultragoal goal state, create worker ledgers, mutate `.nomx/ultragoal`, auto-launch Team from Ultragoal, or perform hidden Codex goal mutation. The leader uses terminal Team evidence plus a fresh `get_goal` snapshot to run `nomx ultragoal checkpoint --goal-id <id> --status complete --evidence "<team evidence mentioning .nomx/ultragoal and <id>>" --codex-goal-json <fresh-get_goal-json-or-path>`.
...
NOMX_TEAM_WORKER_CLI=claude nomx team 2:executor "update docs and report"

# Mixed team (worker 1 = Codex, worker 2 = Claude)
NOMX_TEAM_WORKER_CLI_MAP=codex,claude nomx team 2:executor "split doc/code tasks"

# Auto mode: Claude is selected when worker launch args/model contains 'claude'
NOMX_TEAM_WORKER_CLI=auto NOMX_TEAM_WORKER_LAUNCH_ARGS="--model claude-..." nomx team 2:executor "run mixed validation"
```
...
2. Reuse the latest relevant snapshot in `.nomx/context/{slug}-*.md` when available.
3. If none exists, create `.nomx/context/{slug}-{timestamp}.md` (UTC `YYYYMMDDTHHMMSSZ`) with:
   - task statement
   - desired outcome
...
For simple read-only brownfield lookups during intake, follow active session guidance: when `USE_NOMX_EXPLORE_CMD` is enabled, prefer `nomx explore` with narrow, concrete prompts; otherwise use the richer normal explore path and fall back normally if `nomx explore` is unavailable.
...
3. Initialize team state:
   - `.nomx/state/team/<team>/config.json`
   - `.nomx/state/team/<team>/manifest.v2.json`
   - `.nomx/state/team/<team>/tasks/task-<id>.json`
4. Compose team-scoped worker instructions file at:
   - `.nomx/state/team/<team>/worker-agents.md`
   - Uses project `AGENTS.md` content (if present) + worker overlay, without mutating project `AGENTS.md`
5. Resolve canonical shared state root from leader cwd (`<leader-cwd>/.nomx/state`)
6. Split current tmux window into worker panes
7. Launch workers with:
   - `NOMX_TEAM_WORKER=<team>/worker-<n>`
   - `NOMX_TEAM_STATE_ROOT=<leader-cwd>/.nomx/state`
   - `NOMX_TEAM_LEADER_CWD=<leader-cwd>`
   - worker CLI selected by `NOMX_TEAM_WORKER_CLI` / `NOMX_TEAM_WORKER_CLI_MAP` (`codex` or `claude`)
...
- Submit routing uses this CLI resolution order per worker trigger:
  ...
  2) `NOMX_TEAM_WORKER_CLI_MAP` entry for that worker index,
  3) fallback `NOMX_TEAM_WORKER_CLI` / auto detection.
...
1. Explicit worker model in `NOMX_TEAM_WORKER_LAUNCH_ARGS`
2. Inherited leader `--model` flag
3. Low-complexity default from `NOMX_DEFAULT_SPARK_MODEL` (legacy alias: `NOMX_SPARK_MODEL`) when 1+2 are absent and team `agentType` is low-complexity

Default-model rule:
- Do **not** assume a frontier or spark model from recency or model-family heuristics.
- Use `NOMX_DEFAULT_FRONTIER_MODEL` for frontier-default guidance.
- Use `NOMX_DEFAULT_SPARK_MODEL` for spark/low-complexity worker-default guidance.
...
- When the leader assigns teammate roles/tasks, NOMX allocates **per-worker reasoning effort dynamically** from the resolved worker role and `agentReasoning` overrides (`low`, `medium`, `high`, `xhigh`).
- Explicit launch args still win: if `NOMX_TEAM_WORKER_LAUNCH_ARGS` already includes `-c model_reasoning_effort=...`, that explicit value overrides dynamic allocation for every worker.
...
- `shutdown`: graceful shutdown request, then cleanup (deletes `.nomx/state/team/<team>`)

## Data Plane and Control Plane

### Control Plane

- tmux panes/processes (`NOMX_TEAM_WORKER` per worker)
- leader notifications via `tmux display-message`

### Data Plane

- `.nomx/state/team/<team>/...` files
- Team mailbox files:
- `.nomx/state/team/<team>/mailbox/leader-fixed.json`
- `.nomx/state/team/<team>/mailbox/worker-<n>.json`
- `.nomx/state/team/<team>/dispatch/requests.json` (durable dispatch queue; hook-preferred, fallback-aware)
...
- `.nomx/state/team/<team>/manifest.v2.json`
- `.nomx/state/team/<team>/tasks/task-<id>.json`
- `.nomx/state/team/<team>/workers/worker-<n>/identity.json`
- `.nomx/state/team/<team>/workers/worker-<n>/inbox.md`
- `.nomx/state/team/<team>/workers/worker-<n>/heartbeat.json`
- `.nomx/state/team/<team>/workers/worker-<n>/status.json`
- `.nomx/state/team-leader-nudge.json`
...
2. `.nomx/state/team/<team>/mailbox/leader-fixed.json` exists
...
- Typical cause: leader/manual flow ran `nomx team shutdown <team>` (or removed `.nomx/state/team/<team>`) before worker finished.

Checks:

1. `nomx team status <team>` and confirm whether tasks were still `in_progress` when shutdown occurred
2. Verify whether `.nomx/state/team/<team>/` exists
3. Inspect worker pane tail for post-shutdown writes
4. Confirm no external cleanup (`rm -rf .nomx/state/team/<team>`) happened during execution
...
rm -rf .nomx/state/team/<team-name>
...
- **Team state files** — Inspect `.nomx/state/team/<team>/` when you need status, task, or mailbox evidence after launch.

### Cleanup distinction

Two cleanup paths exist and must not be confused:

- `team_cleanup` (**state-server**): Deletes team state **files** on disk (`.nomx/state/team/<team>/`). Use after a team run is fully complete.
