...
`$ralplan` is a planning mode. While ralplan is active and no explicit execution handoff is active, implementation-focused write tools are out of scope. Ralplan may inspect the repository and may write only planning artifacts such as `.nomx/context/`, `.nomx/plans/`, `.nomx/specs/`, and required `.nomx/state/` records.
...
When ralplan outputs a final handoff or asks the user to choose a next lane, include the retained execution options:
...
4. If ambiguity remains high, gather brownfield facts first. `nomx explore` is deprecated; use normal repository inspection tools/subagents for simple read-only repository lookups and `nomx sparkshell` only for explicit shell-native read-only evidence. Then run `$deep-interview --quick <task>` before continuing.
...
6. Treat any existing approved research artifact as evidence for the plan, but do not introduce a separate research workflow into the final architecture unless the user explicitly requests one.
