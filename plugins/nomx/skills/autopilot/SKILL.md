---
name: autopilot
description: "[NOMX] Strict autonomous loop: $deep-interview -> $ralplan -> $ultragoal -> $code-review -> $ultraqa"
---
...
1. **Phase `deep-interview`** — Socratic requirements clarification gate
   ...
   - After a user answers an interview question, re-score ambiguity against the active profile threshold. Ask another question only when a readiness gate is still unresolved and the answer would materially change execution; otherwise crystallize the spec and hand off.
...
2. **Phase `ralplan`** — consensus planning gate
   ...
   - If either review is missing, blocked, out of order, or non-approving, remain in `ralplan` or report an explicit blocker/max-iteration outcome; do not progress to `$ultragoal`, `$ralph`, or implementation.
...
3. **Phase `ultragoal`** — durable implementation + verification loop
   ...
   - Ultragoal owns durable Codex goal handoffs, `.nomx/ultragoal` ledger checkpoints, implementation, tests, build/lint/typecheck evidence, cleanup, and final review gate discipline.
   - Use native Codex subagents inside an active Ultragoal story when independent file/module lanes, broad test matrices, or multi-domain implementation benefit from parallel execution. The leader owns integration; Ultragoal keeps the goal/ledger state.
...
2. Reuse the latest relevant `.nomx/context/{slug}-*.md` snapshot when available.
...
- Native Codex subagents are conditional: use them only when bounded parallel execution materially improves throughput, quality, or safety.
...
<State_Management>
Use the CLI-first state surface (`nomx state ... --json`) for Autopilot lifecycle state. State must be session-aware when a session id exists. If the explicit MCP compatibility surface is already available, equivalent `nomx_state` tool calls remain acceptable but are not required.
...
- **On cancellation**: run `nomx cancel`; preserve progress for resume rather than deleting handoff artifacts.
</State_Management>
...
- Stop when the user says "stop", "cancel", or "abort" and run `nomx cancel`.
