...
export function buildGoalWorkflowHandoff(options: GoalWorkflowHandoffOptions): string {
  ...
  return [
    ...
    '- Call update_goal({status: "complete"}) only after the NOMX completion audit and validation artifacts pass, then call get_goal again for a fresh complete snapshot.',
    options.completionCommand ? `- Then record NOMX completion evidence with the fresh snapshot: ${options.completionCommand}` : '- Then record NOMX completion evidence in the workflow ledger/status artifacts with the fresh get_goal snapshot when the workflow supports --codex-goal-json.',
    '',
    options.degradedMode
      ...
      : 'Truth boundary: NOMX owns durable workflow artifacts; Codex owns active-thread focus/accounting.',
    '',
    'create_goal payload:',
