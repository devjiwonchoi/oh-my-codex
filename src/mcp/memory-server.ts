/**
 * NOMX Project Memory & Notepad MCP Server
 * Provides persistent project memory and session notepad tools
 * Storage: .nomx/project-memory.json, .nomx/notepad.md
 */
...
function getMemoryPath(wd: string): string {
  return join(wd, '.nomx', 'project-memory.json');
}

function getNotepadPath(wd: string): string {
  return join(wd, '.nomx', 'notepad.md');
}
...
const server = new Server(
  { name: 'nomx-memory', version: '0.1.0' },
  { capabilities: { tools: {} } }
);
