...
describe('mcp/code-intel-server module contract', () => {
  it('declares expected MCP tools and diagnostics command shape', async () => {
    ...
    assert.match(src, /new Server\(\s*\{ name: 'nomx-code-intel', version: '0\.1\.0' \}/);
  });
  ...
  it('skips tsc diagnostics when the project has no tsconfig', async () => {
    const previous = process.env.NOMX_CODE_INTEL_SERVER_DISABLE_AUTO_START;
    process.env.NOMX_CODE_INTEL_SERVER_DISABLE_AUTO_START = '1';

    const projectDir = await mkdtemp(join(tmpdir(), 'nomx-code-intel-'));
    ...
    try {
      ...
      if (previous === undefined) {
        delete process.env.NOMX_CODE_INTEL_SERVER_DISABLE_AUTO_START;
      } else {
        process.env.NOMX_CODE_INTEL_SERVER_DISABLE_AUTO_START = previous;
      }
