...
const SCHEMA = 'nomx.issue3138.installed-smoke.v5';
...
function parseArgs(argv: string[]): SmokeArguments {
  ...
  assert(/(?:^|[\\/])node_modules[\\/]nomx$/.test(packageRoot), '--package-root must name the installed nomx package');
...
async function withWorkspace<T>(name: string, run: (cwd: string, home: string) => Promise<T>): Promise<T> {
  const root = await mkdtemp(join(tmpdir(), `nomx-issue-3138-${name}-`));
  const cwd = join(root, 'project');
  const home = join(root, 'home');
  await mkdir(cwd, { recursive: true });
  await mkdir(home, { recursive: true });
  await mkdir(join(cwd, '.nomx', 'state'), { recursive: true });
  await writeFile(join(cwd, '.nomx', 'managed'), 'issue #3138 installed smoke fixture');
  try {
...
set -eu
case "$*" in
  *"@nomx_pane_instance_id"*) printf '${instanceId}\\n' ;;
  *"@nomx_instance_id"*) printf '${instanceId}\\n' ;;
...
function statePath(cwd: string, sessionId: string, mode: string): string {
  return join(cwd, '.nomx', 'state', 'sessions', sessionId, `${mode}-state.json`);
}
...
async function writeModeState(
  ...
  await mkdir(join(cwd, '.nomx', 'state', 'sessions', sessionId), { recursive: true });
...
async function writeRalplanConsensusEvidence(cwd: string, sessionId: string): Promise<Record<string, unknown>> {
  ...
  await writeFile(join(cwd, '.nomx', 'state', 'subagent-tracking.json'), JSON.stringify({
    schemaVersion: 1,
    sessions: {
      [sessionId]: {
        session_id: sessionId,
  ...
  return {
    ...
    ralplan_architect_review: {
      ...
      thread_id: 'thread-architect', artifact_path: '.nomx/artifacts/architect.md', tracker_path: '.nomx/state/subagent-tracking.json',
    },
    ralplan_critic_review: {
      agent_role: 'critic', verdict: 'approve', provenance_kind: 'native_subagent', session_id: sessionId,
      thread_id: 'thread-critic', artifact_path: '.nomx/artifacts/critic.md', tracker_path: '.nomx/state/subagent-tracking.json',
    },
  };
}
...
function cleanEnv(home: string, cwd: string, extra: NodeJS.ProcessEnv = {}): NodeJS.ProcessEnv {
  return {
    ...
    NOMX_ROOT: cwd,
    NOMX_STATE_ROOT: undefined,
    NOMX_TEAM_STATE_ROOT: undefined,
    NOMX_MCP_WORKDIR_ROOTS: undefined,
...
async function scenarioMissingOwnerAliasActivation(packageRoot: string): Promise<void> {
  await withWorkspace('missing-owner-alias', async (cwd, home) => {
    const ownerSessionId = 'nomx-owner-alias';
    ...
    await withEnvironment(cleanEnv(home, cwd, {
      NOMX_SESSION_ID: ownerSessionId,
      ...
      const pointer = await readJson<Record<string, unknown>>(join(cwd, '.nomx', 'state', 'session.json'));
      ...
      assert(!existsSync(join(cwd, '.nomx', 'state', 'sessions', ownerSessionId)), 'owner alias activation created a split owner scope');
    });
  });
}
...
async function scenarioUnmatchedWrite(packageRoot: string): Promise<void> {
  await withWorkspace('unmatched-write', async (cwd, home) => {
    const canonicalSessionId = 'nomx-canonical-write';
    const ownerSessionId = 'nomx-unmatched-write';
    ...
    ], cleanEnv(home, cwd, { NOMX_SESSION_ID: ownerSessionId, TMUX: '', TMUX_PANE: '' }));
    ...
    assert(!existsSync(join(cwd, '.nomx', 'state', 'sessions', ownerSessionId)), 'unmatched owner state write created an owner scope');
  });
}

async function scenarioOrdinaryClear(packageRoot: string): Promise<void> {
  await withWorkspace('ordinary-clear', async (cwd, home) => {
    const canonicalSessionId = 'nomx-canonical-clear';
    const ownerSessionId = 'nomx-unmatched-clear';
    ...
    assert(!existsSync(join(cwd, '.nomx', 'state', 'sessions', ownerSessionId)), 'ordinary clear created an owner scope');
  });
}

async function scenarioCancel(packageRoot: string): Promise<void> {
  await withWorkspace('cancel', async (cwd, home) => {
    const canonicalSessionId = 'nomx-canonical-cancel';
    const ownerSessionId = 'nomx-unmatched-cancel';
    ...
    assert(!existsSync(join(cwd, '.nomx', 'state', 'sessions', ownerSessionId)), 'cancel created an owner scope');
  });
}

async function scenarioCancelForce(packageRoot: string): Promise<void> {
  await withWorkspace('cancel-force', async (cwd, home) => {
    const canonicalSessionId = 'nomx-canonical-cancel-force';
    const ownerSessionId = 'nomx-unmatched-cancel-force';
    ...
    assert(!existsSync(join(cwd, '.nomx', 'state', 'sessions', ownerSessionId)), 'force cancel created an owner scope');
  });
}
...
async function scenarioOwnerTerminalNativeStop(packageRoot: string): Promise<void> {
  await withWorkspace('owner-terminal-stop', async (cwd, home) => {
    const canonicalSessionId = 'nomx-canonical-terminal';
    const ownerSessionId = 'nomx-owner-terminal';
    ...
    assert(!existsSync(join(cwd, '.nomx', 'state', 'sessions', ownerSessionId)), 'owner terminal write created a split owner scope');
...
async function scenarioAllSessionsClear(packageRoot: string): Promise<void> {
  await withWorkspace('all-sessions-clear', async (cwd, home) => {
    const canonicalSessionId = 'nomx-canonical-all-clear';
    const otherSessionId = 'nomx-existing-other';
    const ownerSessionId = 'nomx-unmatched-all-clear';
    ...
    assert(!existsSync(join(cwd, '.nomx', 'state', 'sessions', ownerSessionId)), 'all-sessions clear initialized an owner scope');
  });
}
...
async function scenarioManagedUnmatchedIdleDeliveryNoReceipts(packageRoot: string): Promise<void> {
  await withWorkspace('managed-unmatched-idle', async (cwd, home) => {
    const ownerSessionId = 'nomx-unmatched-idle';
    await writeNotificationConfig(home);
    await withEnvironment(cleanEnv(home, cwd, { NOMX_SESSION_ID: ownerSessionId, TMUX_PANE: '%issue3138-idle-pane' }), async () => {
    ...
    assert(!existsSync(join(cwd, '.nomx', 'state', 'sessions', ownerSessionId, 'idle-notif-cooldown.json')), 'managed unmatched idle wrote an idle receipt');
    assert(!existsSync(join(cwd, '.nomx', 'state', 'sessions', ownerSessionId, 'lifecycle-notif-state.json')), 'managed unmatched idle wrote a lifecycle receipt');
    assert(!existsSync(join(home, '.nomx', 'state', 'reply-session-registry.jsonl')), 'managed unmatched idle wrote a reply mapping');
    assert(!existsSync(join(home, '.nomx', 'state', 'reply-session-registry.lock')), 'managed unmatched idle left a reply registry lock');
  });
}
...
async function scenarioRejectedRootNoLaunch(packageRoot: string): Promise<void> {
  await withWorkspace('rejected-root', async (cwd, home) => {
    const outsideRoot = await mkdtemp(join(tmpdir(), 'nomx-issue-3138-rejected-root-'));
    const binDir = join(cwd, 'bin');
    ...
    try {
      ...
      const result = runInstalledCli(packageRoot, cwd, ['--dangerously-bypass-approvals-and-sandbox'], cleanEnv(home, cwd, {
        NOMX_ROOT: outsideRoot,
        NOMX_MCP_WORKDIR_ROOTS: cwd,
      ...
      assert(!existsSync(join(cwd, '.nomx', 'state', 'session.json')), 'rejected root committed a session pointer');
      assert(!existsSync(join(cwd, '.nomx', 'runtime')), 'rejected root created post-launch runtime artifacts');
    } finally {
...
async function scenarioManagedUnmatchedLifecycleDeliveryNoReceipts(packageRoot: string): Promise<void> {
  await withWorkspace('managed-unmatched-lifecycle', async (cwd, home) => {
    const ownerSessionId = 'nomx-unmatched-lifecycle';
    await writeNotificationConfig(home);
    await withEnvironment(cleanEnv(home, cwd, { NOMX_SESSION_ID: ownerSessionId }), async () => {
      const notifications = await importInstalled<{
        notifyLifecycle: (
          event: string,
          payload: Record<string, unknown>,
    ...
    assert(!existsSync(join(cwd, '.nomx', 'state', 'sessions', ownerSessionId, 'lifecycle-notif-state.json')), 'managed unmatched lifecycle wrote a lifecycle receipt');
  });
}
...
async function scenarioExplicitPayloadSessionIsolation(packageRoot: string): Promise<void> {
  await withWorkspace('explicit-payload-isolation', async (cwd, home) => {
    ...
    const pointerPath = join(cwd, '.nomx', 'state', 'session.json');
...
async function scenarioNotifyExistingForkPreservesPayloadOwner(packageRoot: string): Promise<void> {
  await withWorkspace('notify-existing-fork-owner', async (cwd, home) => {
    const canonicalSessionId = 'nomx-notify-canonical';
    const payloadSessionId = 'codex-notify-owner';
    const forkSessionId = 'nomx-notify-existing-fork';
    const canonicalHudPath = join(cwd, '.nomx', 'state', 'sessions', canonicalSessionId, 'hud-state.json');
    await writeCanonicalPointer(packageRoot, cwd, home, canonicalSessionId, payloadSessionId);
    await mkdir(join(cwd, '.nomx', 'state', 'sessions', forkSessionId), { recursive: true });
    ...
    })], cleanEnv(home, cwd, { NOMX_SESSION_ID: forkSessionId }));
    assert(result.status === 0, result.stderr || result.stdout || 'installed notify fork route failed');
    const forkDir = join(cwd, '.nomx', 'state', 'sessions', forkSessionId);
    ...
    assert(existsSync(join(cwd, '.nomx', 'state', 'sessions', forkSessionId, 'hud-state.json')), 'notify fork route did not mutate existing fork storage');
    assert(existsSync(join(cwd, '.nomx', 'state', 'sessions', forkSessionId, 'notify-hook-state.json')), 'notify fork route did not persist its fork receipt');
    assert(await readFile(canonicalHudPath, 'utf-8') === canonicalHudBefore, 'notify fork route mutated canonical source storage');
    assert(!existsSync(join(cwd, '.nomx', 'state', 'sessions', canonicalSessionId, 'notify-hook-state.json')), 'notify fork route created a canonical receipt');
    assert(!existsSync(join(cwd, '.nomx', 'state', 'sessions', payloadSessionId)), 'notify fork route created payload owner storage');
