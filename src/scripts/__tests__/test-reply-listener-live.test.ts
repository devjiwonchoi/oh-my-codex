...
test('resolveReplyListenerLiveEnv stays disabled until explicitly opted in', () => {
  const result = resolveReplyListenerLiveEnv({
    NOMX_DISCORD_NOTIFIER_BOT_TOKEN: 'discord-token',
    NOMX_DISCORD_NOTIFIER_CHANNEL: 'channel-1',
    NOMX_TELEGRAM_BOT_TOKEN: 'telegram-token',
    NOMX_TELEGRAM_CHAT_ID: 'chat-1',
  });
