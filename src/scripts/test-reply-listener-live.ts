...
const LIVE_ENABLE_ENV = 'NOMX_REPLY_LISTENER_LIVE';
...
export function resolveReplyListenerLiveEnv(env: NodeJS.ProcessEnv = process.env): ReplyListenerLiveEnvResolution {
  ...
  return {
    ...
    missing: [],
    config: {
      discordBotToken: env.NOMX_DISCORD_NOTIFIER_BOT_TOKEN!.trim(),
      discordChannelId: env.NOMX_DISCORD_NOTIFIER_CHANNEL!.trim(),
      telegramBotToken: env.NOMX_TELEGRAM_BOT_TOKEN!.trim(),
      telegramChatId: env.NOMX_TELEGRAM_CHAT_ID!.trim(),
    },
  };
}
