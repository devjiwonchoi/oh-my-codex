...
export function resolveWorkerIdleNotifyEnabled() {
  const raw = safeString(process.env.NOMX_TEAM_WORKER_IDLE_NOTIFY || '').trim().toLowerCase();
...
export function resolveWorkerIdleCooldownMs() {
  const raw = safeString(process.env.NOMX_TEAM_WORKER_IDLE_COOLDOWN_MS || '');
...
export function resolveAllWorkersIdleCooldownMs() {
  const raw = safeString(process.env.NOMX_TEAM_ALL_IDLE_COOLDOWN_MS || '');
  const parsed = asNumber(raw);
...
export function resolveStatusStaleMs() {
  const raw = safeString(process.env.NOMX_TEAM_STATUS_STALE_MS || '');
  const parsed = asNumber(raw);
...
export function resolveHeartbeatStaleMs() {
  const raw = safeString(process.env.NOMX_TEAM_HEARTBEAT_STALE_MS || '');
  const parsed = asNumber(raw);
  if (parsed !== null && parsed >= 5_000 && parsed <= 60 * 60_000) return parsed;
  return 180_000;
}
...
export async function maybeNotifyLeaderAllWorkersIdle({ cwd, stateDir, logsDir, parsedTeamWorker }) {
  ...
  const nextAction = `Run \`nomx team status ${teamName}\` now, read unread worker messages, then assign the next concrete task, reconcile results, or shut the team down.`;
  const message = `[NOMX] All ${N} worker${N === 1 ? '' : 's'} idle. ${nextAction} ${DEFAULT_MARKER}`;
...
export async function maybeNotifyLeaderWorkerIdle({ cwd, stateDir, logsDir, parsedTeamWorker }) {
  ...
  const parts = [`[NOMX] ${workerName} ${currentState}`];
