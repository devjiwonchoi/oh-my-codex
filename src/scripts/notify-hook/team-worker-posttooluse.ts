...
async function checkpointIfEligible(cwd: string, workerName: string): Promise<{
  ...
  if (checkpointable.length === 0) {
    const onlyRuntimeArtifacts = paths.every((path) => {
      const normalized = path.replace(/\\/g, '/');
      return normalized.startsWith('.nomx/state/') || normalized.startsWith('.nomx/logs/') || normalized.startsWith('.nomx/reports/');
    });
  ...
  const commitResult = await gitMaybe(cwd, ['commit', '--no-verify', '-m', `nomx(team): auto-checkpoint ${workerName}`]);
  if (!commitResult.ok) {
    await unstageCheckpointablePaths(cwd, checkpointable);
...
export async function handleTeamWorkerPostToolUseSuccess(
  ...
  try {
    ...
    const parsedWorker = parseTeamWorkerEnv(env.NOMX_TEAM_INTERNAL_WORKER ?? env.NOMX_TEAM_WORKER);
