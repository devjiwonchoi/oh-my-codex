...
function recordBridgeFallback({
  ...
  try {
    process.emitWarning(`[nomx] team-dispatch bridge fallback: ${event.bridge_operation} -> ${event.fallback_target}: ${event.reason}`, {
      code: 'NOMX_TEAM_DISPATCH_BRIDGE_FALLBACK',
    });
  } catch {
    // best effort observability only
  }
...
const ISSUE_DISPATCH_COOLDOWN_ENV = 'NOMX_TEAM_DISPATCH_ISSUE_COOLDOWN_MS';
const DEFAULT_DISPATCH_TRIGGER_COOLDOWN_MS = 30 * 1000;
const DISPATCH_TRIGGER_COOLDOWN_ENV = 'NOMX_TEAM_DISPATCH_TRIGGER_COOLDOWN_MS';
...
export async function drainPendingTeamDispatch({
  cwd,
  stateDir = join(cwd || process.cwd(), '.nomx', 'state'),
  ...
  if (safeString(process.env.NOMX_TEAM_WORKER)) {
