...
async function initWorkerFixture(): Promise<{ cwd: string; stateRoot: string; env: NodeJS.ProcessEnv }> {
  const cwd = await mkdtemp(join(tmpdir(), 'nomx-posttooluse-worker-'));
  ...
  return {
    ...
    env: {
      ...process.env,
      NOMX_TEAM_WORKER: 'demo-team/worker-1',
...
describe('handleTeamWorkerPostToolUseSuccess', () => {
  it('creates a safe worker checkpoint, ledger entries, leader signal, and dedupe marker', async () => {
    ...
    assert.equal(log, 'nomx(team): auto-checkpoint worker-1');
    ...
    const ledger = JSON.parse(await readFile(join(fixture.cwd, '.nomx', 'reports', 'team-commit-hygiene', 'demo-team.ledger.json'), 'utf-8')) as { entries: Array<{ operation: string; status: string }> };
  ...
  it('exports canonical dedupe and protected-path helpers for fallback suppression', () => {
    ...
    assert.equal(teamWorkerPostToolUseInternals.isProtectedCheckpointPath('.nomx/state/team/x'), true);
  ...
  it('fails closed for missing worker identity without guessed state writes', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-posttooluse-missing-identity-'));
    const result = await handleTeamWorkerPostToolUseSuccess(successPayload, cwd, {
      ...process.env,
      NOMX_TEAM_WORKER: 'demo-team/worker-1',
      NOMX_TEAM_STATE_ROOT: join(cwd, '.nomx', 'state'),
    });

    assert.equal(result.handled, false);
    assert.equal(result.status, 'skipped');
    assert.equal(existsSync(join(cwd, '.nomx', 'state', 'team')), false);
  });
});
