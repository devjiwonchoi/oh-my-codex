...
async function withTempWorkingDir(run: (cwd: string) => Promise<void>): Promise<void> {
  const cwd = await mkdtemp(join(tmpdir(), 'nomx-notify-all-idle-'));
  try {
    await run(cwd);
  } finally {
...
function writeWorkerIdentityFixture(cwd: string, workerEnv: string): string {
  ...
  const stateRoot = join(cwd, '.nomx', 'state');
...
function runNotifyHookAsWorker(
  ...
  return spawnSync(process.execPath, [NOTIFY_HOOK_SCRIPT.pathname, JSON.stringify(payload)], {
    ...
    env: {
      ...
      NOMX_TEAM_WORKER: workerEnv,
      NOMX_TEAM_STATE_ROOT: stateRoot,
      NOMX_TEAM_LEADER_CWD: '',
      NOMX_MODEL_INSTRUCTIONS_FILE: '',
      NOMX_TEAM_WORKER_IDLE_NOTIFY: 'false',
      NOMX_TEAM_ALL_IDLE_COOLDOWN_MS: '500', // short cooldown for tests
      TMUX: '',
      TMUX_PANE: '',
      ...extraEnv,
    },
...
describe('notify-hook all-workers-idle notification', () => {
  ...
  it('writes deferred visibility once per cooldown window when leader pane is missing', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const first = runNotifyHookAsWorker(cwd, fakeBinDir, `${teamName}/worker-1`, { NOMX_TEAM_ALL_IDLE_COOLDOWN_MS: '600000' });
      assert.equal(first.status, 0, `notify-hook failed: ${first.stderr || first.stdout}`);
      const second = runNotifyHookAsWorker(cwd, fakeBinDir, `${teamName}/worker-1`, { NOMX_TEAM_ALL_IDLE_COOLDOWN_MS: '600000' });
...
`;
  ...
  it('does not notify when some workers are still working', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      if (existsSync(tmuxLogPath)) {
        const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
        assert.doesNotMatch(tmuxLog, /\[NOMX\] All .* idle/, 'should NOT send all-idle message when some workers are busy');
      }
    });
  });
  ...
  it('does not notify when a worker heartbeat is stale', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      if (existsSync(tmuxLogPath)) {
        const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
        assert.doesNotMatch(tmuxLog, /\[NOMX\] All .* idle/, 'stale heartbeat should suppress all-workers-idle notification');
      }
    });
  });
  ...
  it('does not notify when current worker is not idle', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      if (existsSync(tmuxLogPath)) {
        const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
        assert.doesNotMatch(tmuxLog, /\[NOMX\] All .* idle/);
      }
    });
  });
  ...
  it('respects cooldown: does not send repeated notifications', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const result = runNotifyHookAsWorker(cwd, fakeBinDir, `${teamName}/worker-1`, {
        NOMX_TEAM_ALL_IDLE_COOLDOWN_MS: '600000',
      });
      assert.equal(result.status, 0, `notify-hook failed: ${result.stderr || result.stdout}`);

      if (existsSync(tmuxLogPath)) {
        const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
        assert.doesNotMatch(tmuxLog, /\[NOMX\] All .* idle/, 'cooldown should block repeated notification');
      }
    });
  });
  ...
  it('does not fire for leader (non-team-worker) context', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      // Run as LEADER (no NOMX_TEAM_WORKER env var)
      const payload = {
        cwd,
        type: 'agent-turn-complete',
      ...
      const result = spawnSync(process.execPath, [NOTIFY_HOOK_SCRIPT.pathname, JSON.stringify(payload)], {
        ...
        env: {
          ...process.env,
          PATH: `${fakeBinDir}:${process.env.PATH || ''}`,
          NOMX_TEAM_WORKER: '', // empty = not a worker
      ...
      if (existsSync(tmuxLogPath)) {
        const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
        assert.doesNotMatch(tmuxLog, /\[NOMX\] All .* idle/, 'leader context should not send all-idle notification');
      }
    });
  });
  ...
  it('handles single worker team correctly with singular message', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
      assert.match(tmuxLog, /\[NOMX\] All 1 worker idle/, 'single worker uses singular form');
      assert.match(tmuxLog, /Run `nomx team status solo-team` now, read unread worker messages, then assign the next concrete task, reconcile results, or shut the team down/, 'all-workers-idle notification should tell the agent to execute the runtime status check directly');
      assert.doesNotMatch(tmuxLog, /Next: run nomx team status solo-team, read unread worker messages, then decide whether to assign the next concrete task, reconcile results, or shut the team down/, 'all-workers-idle notification should not fall back to human-advisory wording');
