...
describe('normalizeTmuxHookConfig', () => {
  ...
  it('normalizes valid config', () => {
    const config = normalizeTmuxHookConfig({
      enabled: true,
      target: { type: 'session', value: 'nomx-work' },
      allowed_modes: ['team'],
      cooldown_ms: 2000,
    ...
    assert.deepEqual(config.target, { type: 'session', value: 'nomx-work' });
...
describe('evaluateInjectionGuards', () => {
  const validConfig = normalizeTmuxHookConfig({
    enabled: true,
    target: { type: 'session', value: 'nomx' },
    allowed_modes: ['ralph'],
    cooldown_ms: 1000,
...
describe('resolveCodexPane', () => {
  it('ignores HUD pane even when TMUX_PANE foreground command is node', async () => {
    ...
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'nomx-resolve-codex-pane-'));
