...
async function listDispatchProcessingLeases(cwd: string, teamName: string): Promise<string[]> {
  const dispatchDir = join(cwd, '.nomx', 'state', 'team', teamName, 'dispatch');
...
describe('notify-hook team dispatch consumer', () => {
  ...
  after(() => {
    if (originalTeamWorker === undefined) {
      ...
      process.env.NOMX_TEAM_WORKER = originalTeamWorker;
    }

    if (originalTeamStateRoot === undefined) {
      delete process.env.NOMX_TEAM_STATE_ROOT;
    } else {
      process.env.NOMX_TEAM_STATE_ROOT = originalTeamStateRoot;
    }
  });

  it('marks pending request as notified and preserves mailbox notified_at semantics', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-hook-team-dispatch-'));
    try {
  ...
  it('leader-fixed dispatch remains pending with leader_pane_missing_deferred when pane missing', async () => {
    ...
    try {
      ...
      const eventsPath = join(cwd, '.nomx', 'state', 'team', 'alpha', 'events', 'events.ndjson');
      ...
      const dispatchLogPath = join(cwd, '.nomx', 'logs', `team-dispatch-${new Date().toISOString().slice(0, 10)}.jsonl`);
  ...
  it('marks bridge-authored mailbox state as notified on canonical hook success paths', async () => {
    ...
    const runtimePath = join(fakeBinDir, 'nomx-runtime');
    const previousPath = process.env.PATH;
    const previousRuntimeBinary = process.env.NOMX_RUNTIME_BINARY;
    ...
    try {
      ...
      if (typeof previousRuntimeBinary === 'string') process.env.NOMX_RUNTIME_BINARY = previousRuntimeBinary;
  ...
  it('logs structured evidence when bridge dispatch compat cannot be parsed and legacy requests are used', async () => {
    ...
    try {
      ...
      process.env.NOMX_RUNTIME_BRIDGE = '1';
      await writeFile(join(cwd, '.nomx', 'state', 'dispatch.json'), '{ broken bridge json');
  ...
  it('leader-fixed dispatch uses pane target only when leader_pane_id exists', async () => {
    ...
    try {
      ...
      await enqueueDispatchRequest('alpha', {
        ...
        to_worker: 'leader-fixed',
        message_id: msg.message_id,
        trigger_message: 'Read .nomx/state/team/alpha/mailbox/leader-fixed.json; worker-1 sent a new message. Review it and decide the next concrete step.',
      }, cwd);
...
if [[ "$cmd" == "list-panes" ]]; then
  printf "%%42\\t1\\tnode\\tcodex\\n%%91\\t0\\tnode\\tnode dist/cli/nomx.js hud --watch\\n"
  exit 0
fi
exit 0
`;
...
`,
  ...
  it('uses explicit stateDir when marking mailbox notified_at', async () => {
    ...
    const previousStateRoot = process.env.NOMX_TEAM_STATE_ROOT;
    try {
      process.env.NOMX_TEAM_STATE_ROOT = './custom-state-root';
      await initTeamState('alpha', 'task', 'executor', 1, cwd);
      const msg = await sendDirectMessage('alpha', 'worker-1', 'worker-1', 'hello', cwd);
      const queued = await enqueueDispatchRequest('alpha', {
        kind: 'mailbox',
      ...
      if (typeof previousStateRoot === 'string') process.env.NOMX_TEAM_STATE_ROOT = previousStateRoot;
  ...
  it('releases the global dispatch lock before slow tmux injection so mailbox sends do not wedge mid-run', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-hook-team-dispatch-'));
    const previousLockTimeout = process.env.NOMX_DISPATCH_LOCK_TIMEOUT_MS;
    const previousRuntimeBridge = process.env.NOMX_RUNTIME_BRIDGE;
    try {
      ...
      const requests = JSON.parse(
        await readFile(join(cwd, '.nomx', 'state', 'team', 'alpha', 'dispatch', 'requests.json'), 'utf-8'),
      ...
      if (typeof previousLockTimeout === 'string') process.env.NOMX_DISPATCH_LOCK_TIMEOUT_MS = previousLockTimeout;
      else delete process.env.NOMX_DISPATCH_LOCK_TIMEOUT_MS;
      if (typeof previousRuntimeBridge === 'string') process.env.NOMX_RUNTIME_BRIDGE = previousRuntimeBridge;
      else delete process.env.NOMX_RUNTIME_BRIDGE;
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it('reserves per-issue cooldown before releasing the dispatch lock to a concurrent drain', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-hook-team-dispatch-'));
    const previousIssueCooldown = process.env.NOMX_TEAM_DISPATCH_ISSUE_COOLDOWN_MS;
    ...
    let injectCount = 0;
    try {
      process.env.NOMX_TEAM_DISPATCH_ISSUE_COOLDOWN_MS = '900000';
      ...
      releaseInjector();
      if (typeof previousIssueCooldown === 'string') process.env.NOMX_TEAM_DISPATCH_ISSUE_COOLDOWN_MS = previousIssueCooldown;
      else delete process.env.NOMX_TEAM_DISPATCH_ISSUE_COOLDOWN_MS;
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('keeps retry_pending derived-only and does not persist transient tags', async () => {
    ...
    try {
      ...
      const rawRequests = JSON.parse(await readFile(join(cwd, '.nomx', 'state', 'team', 'alpha', 'dispatch', 'requests.json'), 'utf8'));
  ...
  it('retries submit with isolated C-m and does not retype when trigger already present', async () => {
    ...
    try {
      ...
      process.env.PATH = `${fakeBinDir}:${previousPath || ''}`;
      process.env.NOMX_TEST_CAPTURE_FILE = captureFile;
      ...
      delete process.env.NOMX_TEST_CAPTURE_FILE;
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('retypes on every retry when trigger is not in narrow input area', async () => {
    ...
    try {
      ...
      process.env.PATH = `${fakeBinDir}:${previousPath || ''}`;
      process.env.NOMX_TEST_CAPTURE_SEQUENCE_FILE = captureSeqFile;
      process.env.NOMX_TEST_CAPTURE_COUNTER_FILE = captureCounterFile;
      ...
      delete process.env.NOMX_TEST_CAPTURE_SEQUENCE_FILE;
      delete process.env.NOMX_TEST_CAPTURE_COUNTER_FILE;
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('skips repeated same-issue reinjection during per-issue cooldown window', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-hook-team-dispatch-'));
    const previousCooldown = process.env.NOMX_TEAM_DISPATCH_ISSUE_COOLDOWN_MS;
    let injectCount = 0;
    try {
      process.env.NOMX_TEAM_DISPATCH_ISSUE_COOLDOWN_MS = '900000';
      await initTeamState('alpha', 'task', 'executor', 1, cwd);
      ...
      if (typeof previousCooldown === 'string') process.env.NOMX_TEAM_DISPATCH_ISSUE_COOLDOWN_MS = previousCooldown;
      else delete process.env.NOMX_TEAM_DISPATCH_ISSUE_COOLDOWN_MS;
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('resolves session-only dispatch targets without managed leader session context', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-dispatch-session-target-'));
    ...
    try {
      ...
      assert.ok(cfg);
      if (!cfg) throw new Error('missing team config');
      cfg.tmux_session = 'nomx-team-session-target';
      ...
      assert.match(tmuxLog, /list-panes -t nomx-team-session-target/);
