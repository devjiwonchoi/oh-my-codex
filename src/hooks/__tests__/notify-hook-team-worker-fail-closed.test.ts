...
describe('notify-hook team worker state-root fail-closed behavior', () => {
  it('does not create local team state when worker identity resolution fails', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-notify-worker-fail-closed-'));
    const result = runNotifyHook(cwd, {
      ...process.env,
      NOMX_TEAM_WORKER: 'demo-team/worker-1',
      NOMX_TEAM_STATE_ROOT: join(cwd, 'missing-shared-state'),
    });

    assert.equal(result.status, 0, result.stderr || result.stdout);
    assert.equal(existsSync(join(cwd, '.nomx', 'state')), false);
    assert.equal(existsSync(join(cwd, '.nomx', 'logs')), true);
  });
});
