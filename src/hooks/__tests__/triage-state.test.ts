...
function expectedPath(cwd: string, sessionId?: string): string {
  if (sessionId) {
    return join(cwd, '.nomx', 'state', 'sessions', sessionId, 'prompt-routing-state.json');
  }
  return join(cwd, '.nomx', 'state', 'prompt-routing-state.json');
}
...
describe('readTriageState — malformed JSON returns null', () => {
  ...
  before(() => {
    ...
    mkdirSync(join(tmp, '.nomx', 'state', 'sessions', 'sess-malformed'), { recursive: true });
...
describe('readTriageState — wrong shape returns null', () => {
  ...
  it('returns null for {version:2} (wrong version)', () => {
    ...
    mkdirSync(join(tmp, '.nomx', 'state', 'sessions', 'v2'), { recursive: true });
  ...
  it('returns null for {} (empty object)', () => {
    ...
    mkdirSync(join(tmp, '.nomx', 'state', 'sessions', 'empty'), { recursive: true });
  ...
  it('returns null for missing suppress_followup field', () => {
    ...
    mkdirSync(join(tmp, '.nomx', 'state', 'sessions', 'missing-field'), { recursive: true });
...
describe('readTriageState — valid file returns parsed object', () => {
  ...
  before(() => {
    ...
    mkdirSync(join(tmp, '.nomx', 'state', 'sessions', sessionId), { recursive: true });
  ...
  it('valid state with last_triage=null parses correctly', () => {
    ...
    mkdirSync(join(tmp, '.nomx', 'state', 'sessions', 'null-triage'), { recursive: true });
...
describe('path selection — session-scoped vs root', () => {
  ...
  it('write without sessionId places file at root .nomx/state/prompt-routing-state.json', () => {
  ...
  it('write with malformed explicit sessionId skips persistence instead of falling back to root', () => {
    ...
    try {
      ...
      assert.equal(existsSync(join(isolated, '.nomx', 'state', 'sessions', 'bad', 'session')), false);
    } finally {
      removeTmp(isolated);
    }
  });
...
describe('writeTriageState — directory creation', () => {
  ...
  it('creates parent directories if they do not exist', () => {
    ...
    const dirPath = join(tmp, '.nomx', 'state', 'sessions', sessionId);
