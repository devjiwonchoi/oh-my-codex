...
async function withTempWorkingDir(run: (cwd: string) => Promise<void>): Promise<void> {
  const cwd = await mkdtemp(join(tmpdir(), 'nomx-regression-205-'));
  try {
    await run(cwd);
  } finally {
...
function runNotifyHook(
  ...
  return spawnSync(process.execPath, [NOTIFY_HOOK_SCRIPT.pathname, JSON.stringify(payload)], {
    ...
    env: {
      ...
      NOMX_SESSION_ID: 'sess-managed-regression',
      NOMX_TEST_TMUX_SESSION_NAME: buildTmuxSessionName(cwd, 'sess-managed-regression'),
      TMUX_PANE: '%99',
      TMUX: '1',
      NOMX_TEAM_WORKER: '',
...
describe('regression-205: detectStallPattern excludes "if you want" after #1416', () => {
  ...
  it('ignores NOMX injection-marker lines when matching patterns', async () => {
    const { detectStallPattern, DEFAULT_STALL_PATTERNS } = await loadModule('notify-hook/auto-nudge.js');
    assert.equal(
      detectStallPattern('keep going [NOMX_TMUX_INJECT]', DEFAULT_STALL_PATTERNS),
      false,
    );
  });
...
describe('regression-205: notify-hook ignores "if you want" for default auto-nudge', () => {
  ...
  before(() => {
    originalTeamWorker = process.env.NOMX_TEAM_WORKER;
    originalTeamStateRoot = process.env.NOMX_TEAM_STATE_ROOT;
    delete process.env.NOMX_TEAM_WORKER;
    delete process.env.NOMX_TEAM_STATE_ROOT;
  });

  after(() => {
    if (originalTeamWorker === undefined) delete process.env.NOMX_TEAM_WORKER;
    else process.env.NOMX_TEAM_WORKER = originalTeamWorker;
  ...
  it('does not record pending stall state for permission-seeking "if you want" text', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.doesNotMatch(
        tmuxLog,
        /send-keys -t %99 -l yes, proceed \[NOMX_TMUX_INJECT\]/,
...
describe('regression-205: resolveTeamStateDirForWorker is exported from team-worker.js', () => {
  ...
  it('fails closed for NOMX_TEAM_STATE_ROOT when worker identity is missing', async () => {
    const { resolveTeamStateDirForWorker } = await loadModule('notify-hook/team-worker.js');
    const saved = process.env.NOMX_TEAM_STATE_ROOT;
    ...
    try {
      ...
      if (saved === undefined) {
        ...
        process.env.NOMX_TEAM_STATE_ROOT = saved;
      }
    }
  });

  it('does not guess {cwd}/.nomx/state when no worker identity exists', async () => {
    const { resolveTeamStateDirForWorker } = await loadModule('notify-hook/team-worker.js');
    const savedRoot = process.env.NOMX_TEAM_STATE_ROOT;
    const savedLeader = process.env.NOMX_TEAM_LEADER_CWD;
    delete process.env.NOMX_TEAM_STATE_ROOT;
    ...
    try {
      ...
      if (savedRoot === undefined) {
        ...
        process.env.NOMX_TEAM_STATE_ROOT = savedRoot;
      }
      if (savedLeader === undefined) {
        delete process.env.NOMX_TEAM_LEADER_CWD;
      } else {
        process.env.NOMX_TEAM_LEADER_CWD = savedLeader;
      }
    }
  });
});
