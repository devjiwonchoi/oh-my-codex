...
function isHudStartCommand(startCommand: string): boolean {
  return /\bnomx\b.*\bhud\b.*--watch/i.test(startCommand);
}
...
async function sendTmuxKeys(
  ...
  const marker = process.env.NOMX_HOOK_PLUGIN_LOOP_MARKER || '[NOMX_HOOK_PLUGIN]';
  ...
  const cooldownMs = typeof options.cooldownMs === 'number'
    ...
    : asPositiveNumber(process.env.NOMX_HOOK_PLUGIN_COOLDOWN_MS, DEFAULT_COOLDOWN_MS);
  const dedupeWindowMs = asPositiveNumber(process.env.NOMX_HOOK_PLUGIN_DEDUPE_MS, DEFAULT_DEDUPE_WINDOW_MS);
