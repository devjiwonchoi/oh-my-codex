...
const RESULT_PREFIX = "__NOMX_PLUGIN_RESULT__ ";
const RUNNER_SIGKILL_GRACE_MS = 250;

function hooksLogPath(cwd: string): string {
	const day = new Date().toISOString().slice(0, 10);
	return join(nomxRoot(cwd), "logs", `hooks-${day}.jsonl`);
}
...
async function appendHooksLog(
	...
	await mkdir(join(nomxRoot(cwd), "logs"), { recursive: true });
	await appendFile(
		hooksLogPath(cwd),
		`${JSON.stringify({ timestamp: new Date().toISOString(), ...payload })}\n`,
	).catch((error: unknown) => {
		console.warn("[nomx] warning: failed to append hook dispatch log entry", {
			cwd,
...
function isTeamWorker(env: NodeJS.ProcessEnv): boolean {
	return (
		typeof env.NOMX_TEAM_WORKER === "string" && env.NOMX_TEAM_WORKER.trim() !== ""
	);
}
