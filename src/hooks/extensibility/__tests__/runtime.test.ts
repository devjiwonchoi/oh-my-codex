...
describe('dispatchHookEventRuntime', () => {
  it('dispatches native events even when plugins env var is not set', async () => {
    const originalEnv = process.env.NOMX_HOOK_PLUGINS;
    try {
      delete process.env.NOMX_HOOK_PLUGINS;
      ...
      if (originalEnv !== undefined) {
        process.env.NOMX_HOOK_PLUGINS = originalEnv;
      } else {
        delete process.env.NOMX_HOOK_PLUGINS;
      }
    }
  ...
  it('dispatches when plugins are enabled', async () => {
    const originalEnv = process.env.NOMX_HOOK_PLUGINS;
    try {
      process.env.NOMX_HOOK_PLUGINS = '1';
  ...
  it('marks active leader-owned teams when a native stop event is dispatched without inventing leader attention', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-runtime-stop-team-'));
    try {
  ...
  it('routes native stop leader attention by canonical NOMX session id while preserving native metadata in context', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-runtime-stop-team-native-meta-'));
    try {
      ...
      await writeTeamManifestV2({
        ...
        leader: {
          ...manifest!.leader,
          session_id: 'nomx-canonical-session',
        },
      }, cwd);
      ...
      assert.equal(result.dispatched, true);
      assert.equal(attention?.source, 'native_stop');
      assert.equal(attention?.leader_session_id, 'nomx-canonical-session');
      assert.equal(attention?.leader_session_active, false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
