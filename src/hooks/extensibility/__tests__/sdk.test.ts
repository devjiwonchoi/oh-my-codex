...
async function writeNomxStateFile(cwd: string, fileName: string, value: unknown): Promise<void> {
...
describe('createHookPluginSdk', () => {
  describe('state', () => {
    it('reads undefined for missing key', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'nomx-sdk-'));
      try {
  ...
  describe('tmux.sendKeys', () => {
    ...
    it('returns loop_guard_input_marker when text contains loop marker', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'nomx-sdk-'));
      const originalMarker = process.env.NOMX_HOOK_PLUGIN_LOOP_MARKER;
      try {
        process.env.NOMX_HOOK_PLUGIN_LOOP_MARKER = '[TESTMARK]';
        const sdk = createHookPluginSdk({
          cwd,
          pluginName: 'test',
          event: makeEvent(),
        ...
        if (originalMarker === undefined) {
          delete process.env.NOMX_HOOK_PLUGIN_LOOP_MARKER;
        } else {
          process.env.NOMX_HOOK_PLUGIN_LOOP_MARKER = originalMarker;
        }
        await rm(cwd, { recursive: true, force: true });
      }
    });
    ...
    it('prefers non-HUD codex pane when targeting a tmux session', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'nomx-sdk-'));
      const fakeBinDir = await mkdtemp(join(tmpdir(), 'nomx-sdk-bin-'));
  ...
  describe('nomx', () => {
    it('exposes only the explicit read-only nomx readers', async () => {
      ...
      try {
        ...
        assert.deepEqual(Object.keys(sdk.nomx).sort(), ['hud', 'notifyFallback', 'session', 'updateCheck']);
        assert.equal(typeof sdk.nomx.session.read, 'function');
        assert.equal(typeof sdk.nomx.hud.read, 'function');
        assert.equal(typeof sdk.nomx.notifyFallback.read, 'function');
        assert.equal(typeof sdk.nomx.updateCheck.read, 'function');
        assert.equal('pluginState' in sdk, false);
        assert.equal('readJson' in sdk.nomx, false);
        assert.equal('list' in sdk.nomx, false);
        assert.equal('exists' in sdk.nomx, false);
        assert.equal('write' in sdk.nomx.session, false);
        assert.equal('delete' in sdk.nomx.session, false);
        assert.equal('write' in sdk.nomx.hud, false);
        assert.equal('delete' in sdk.nomx.hud, false);
        assert.equal('write' in sdk.nomx.notifyFallback, false);
        assert.equal('delete' in sdk.nomx.notifyFallback, false);
        assert.equal('write' in sdk.nomx.updateCheck, false);
        assert.equal('delete' in sdk.nomx.updateCheck, false);
      } finally {
        await rm(cwd, { recursive: true, force: true });
      }
    });

    it('reads session state from .nomx/state/session.json', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'nomx-sdk-'));
      try {
        await writeNomxStateFile(cwd, 'session.json', {
        ...
        const sdk = createHookPluginSdk({ cwd, pluginName: 'test', event: makeEvent() });
        const state = await sdk.nomx.session.read();
    ...
    it('returns null for invalid session state without session_id', async () => {
      ...
      try {
        ...
        const sdk = createHookPluginSdk({ cwd, pluginName: 'test', event: makeEvent() });
        assert.equal(await sdk.nomx.session.read(), null);
      } finally {
        await rm(cwd, { recursive: true, force: true });
      }
    });
    ...
    it('reads hud, notifyFallback, and updateCheck state from root-scoped nomx files', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'nomx-sdk-'));
      try {
        await writeNomxStateFile(cwd, 'hud-state.json', {
          last_turn_at: '2026-01-01T00:00:00.000Z',
          turn_count: 3,
        });
        await writeNomxStateFile(cwd, 'notify-fallback-state.json', {
          pid: 1234,
          stopping: false,
          tracked_files: 2,
        });
        await writeNomxStateFile(cwd, 'update-check.json', {
          last_checked_at: '2026-01-01T00:00:00.000Z',
          last_seen_latest: '0.11.0',
        });

        const sdk = createHookPluginSdk({ cwd, pluginName: 'test', event: makeEvent() });
        assert.deepEqual(await sdk.nomx.hud.read(), {
          last_turn_at: '2026-01-01T00:00:00.000Z',
          turn_count: 3,
        });
        assert.deepEqual(await sdk.nomx.notifyFallback.read(), {
          pid: 1234,
          stopping: false,
          tracked_files: 2,
        });
        assert.deepEqual(await sdk.nomx.updateCheck.read(), {
          last_checked_at: '2026-01-01T00:00:00.000Z',
          last_seen_latest: '0.11.0',
        });
      } finally {
    ...
    it('reads hud state from the current session scope instead of stale root fallback', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'nomx-sdk-hud-session-'));
      try {
        await writeNomxStateFile(cwd, 'session.json', {
        ...
        await writeNomxStateFile(cwd, join('sessions', 'sess-current', 'hud-state.json'), {
          last_turn_at: '2026-01-01T00:00:00.000Z',
          turn_count: 3,
    ...
    it('returns null for missing nomx reader files', async () => {
      ...
      try {
        const sdk = createHookPluginSdk({ cwd, pluginName: 'test', event: makeEvent() });
        assert.equal(await sdk.nomx.session.read(), null);
        assert.equal(await sdk.nomx.hud.read(), null);
        assert.equal(await sdk.nomx.notifyFallback.read(), null);
        assert.equal(await sdk.nomx.updateCheck.read(), null);
      } finally {
        await rm(cwd, { recursive: true, force: true });
      }
    });
...
describe('clearHookPluginState', () => {
  it('removes data.json and tmux.json for plugin', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-clear-'));
    try {
      const pluginDir = join(cwd, '.nomx', 'state', 'hooks', 'plugins', 'my-plugin');
