...
function makeTmpDir(): string {
  const dir = join(tmpdir(), `nomx-cs-test-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
...
function initGitRepo(dir: string): void {
  ...
  execSync('git config user.email "nomx-test@example.com"', { cwd: dir, stdio: 'ignore' });
  execSync('git config user.name "NOMX Test"', { cwd: dir, stdio: 'ignore' });
...
function writeEnabledCodeSimplifierConfig(homeDir: string): void {
  const configDir = join(homeDir, '.nomx');
