...
/** Global NOMX config shape (subset relevant to code-simplifier) */
interface NomxGlobalConfig {
...
/**
 * Read the global NOMX config from ~/.nomx/config.json.
 * Returns null if the file does not exist or cannot be parsed.
 *
 * @param configDir - Optional override for the home directory. When provided,
 *   the config is read from `<configDir>/.nomx/config.json` instead of
 *   `~/.nomx/config.json`. Useful for testing without relying on `os.homedir()`.
 */
export function readNomxConfig(configDir?: string): NomxGlobalConfig | null {
  const configPath = join(configDir ?? homedir(), '.nomx', 'config.json');
  ...
  try {
    return JSON.parse(readFileSync(configPath, 'utf-8')) as NomxGlobalConfig;
  } catch {
    return null;
  }
}
...
export function isCodeSimplifierEnabled(configDir?: string): boolean {
  const config = readNomxConfig(configDir);
