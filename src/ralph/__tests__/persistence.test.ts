...
describe('ensureCanonicalRalphArtifacts', () => {
  it('keeps canonical files authoritative when they already exist', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-ralph-canonical-'));
    try {
      const canonicalPrd = join(cwd, '.nomx', 'plans', 'prd-existing.md');
      const canonicalProgress = join(cwd, '.nomx', 'state', 'ralph-progress.json');
      ...
      await writeFile(join(cwd, '.nomx', 'prd.json'), JSON.stringify({ project: 'legacy-project' }));
      await writeFile(join(cwd, '.nomx', 'progress.txt'), 'legacy line\n');
  ...
  it('migrates legacy PRD/progress files one-way when canonical artifacts are absent', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-ralph-migrate-'));
    try {
      const legacyPrdPath = join(cwd, '.nomx', 'prd.json');
      const legacyProgressPath = join(cwd, '.nomx', 'progress.txt');
      ...
      assert.match(canonicalPrd, /Migrated from legacy `.nomx\/prd\.json`/);
      assert.equal(canonicalProgress.source, '.nomx/progress.txt');
  ...
  it('prefers the newest timestamped canonical PRD when multiple canonical files exist', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-ralph-canonical-order-'));
    try {
  ...
  it('records visual feedback with numeric and qualitative guidance for the next iteration', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-ralph-visual-feedback-'));
    try {
