...
export const NOMX_MODELS_START_MARKER = '<!-- NOMX:MODELS:START -->';
export const NOMX_MODELS_END_MARKER = '<!-- NOMX:MODELS:END -->';
...
export function buildAgentsModelTable(
  ...
  return [
    ...
    '',
    'Auto-generated by `nomx setup` from the current `config.toml` plus NOMX model overrides.',
    '',
...
export function upsertAgentsModelTable(
  ...
  const startIndex = content.indexOf(NOMX_MODELS_START_MARKER);
  const endIndex = content.indexOf(NOMX_MODELS_END_MARKER);

  if (startIndex >= 0 && endIndex > startIndex) {
    const replaceEnd = endIndex + NOMX_MODELS_END_MARKER.length;
