...
async function makeRepo(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'nomx-question-policy-'));
  tempDirs.push(cwd);
...
describe('evaluateQuestionPolicy', { concurrency: false }, () => {
  it('allows non-team leader sessions with no blocked modes', { concurrency: false }, async () => {
    ...
    await writeFile(join(cwd, '.nomx', 'state', 'session.json'), JSON.stringify({ session_id: 'sess-1' }));
    const result = await evaluateQuestionPolicy({ cwd, explicitSessionId: 'sess-1', env: { ...process.env, NOMX_TEAM_WORKER: '' } });
    assert.equal(result.allowed, true);
  });

  it('blocks worker contexts immediately', { concurrency: false }, async () => {
    const cwd = await makeRepo();
    const result = await evaluateQuestionPolicy({ cwd, explicitSessionId: 'sess-1', env: { ...process.env, NOMX_TEAM_WORKER: 'demo/worker-1' } });
  ...
  it('blocks canonical active team ownership for the current session', { concurrency: false }, async () => {
    const cwd = await makeRepo();
    const teamRoot = join(cwd, '.nomx', 'state', 'team', 'alpha');
    ...
    await writeFile(join(cwd, '.nomx', 'state', 'session.json'), JSON.stringify({ session_id: 'sess-team' }));
    const result = await evaluateQuestionPolicy({ cwd, explicitSessionId: 'sess-team', env: { ...process.env, NOMX_TEAM_WORKER: '' } });
    assert.equal(result.allowed, false);
  ...
  it('blocks active execution-like workflows for the current session', { concurrency: false }, async () => {
    const cwd = await makeRepo();
    const sessionDir = join(cwd, '.nomx', 'state', 'sessions', 'sess-ralph');
    await mkdir(sessionDir, { recursive: true });
    await writeFile(join(sessionDir, 'ralph-state.json'), JSON.stringify({ mode: 'ralph', active: true }));
    await writeFile(join(cwd, '.nomx', 'state', 'session.json'), JSON.stringify({ session_id: 'sess-ralph' }));
    const result = await evaluateQuestionPolicy({ cwd, explicitSessionId: 'sess-ralph', env: { ...process.env, NOMX_TEAM_WORKER: '' } });
    assert.equal(result.allowed, false);
  ...
  it('does not falsely block from another session team state', { concurrency: false }, async () => {
    const cwd = await makeRepo();
    const teamRoot = join(cwd, '.nomx', 'state', 'team', 'beta');
    await mkdir(teamRoot, { recursive: true });
    await writeFile(join(teamRoot, 'manifest.v2.json'), JSON.stringify({
      schema_version: 2,
      name: 'beta',
    ...
    await writeFile(join(teamRoot, 'phase.json'), JSON.stringify({ current_phase: 'team-exec', max_fix_attempts: 3, current_fix_attempt: 0, transitions: [], updated_at: new Date().toISOString() }));
    const result = await evaluateQuestionPolicy({ cwd, explicitSessionId: 'sess-main', env: { ...process.env, NOMX_TEAM_WORKER: '' } });
    assert.equal(result.allowed, true);
  });

  it('allows deep-interview state when no execution-like workflow is active', { concurrency: false }, async () => {
    const cwd = await makeRepo();
    const sessionDir = join(cwd, '.nomx', 'state', 'sessions', 'sess-di');
    await mkdir(sessionDir, { recursive: true });
    await writeFile(join(sessionDir, 'deep-interview-state.json'), JSON.stringify({ mode: 'deep-interview', active: true }));
    const result = await evaluateQuestionPolicy({ cwd, explicitSessionId: 'sess-di', env: { ...process.env, NOMX_TEAM_WORKER: '' } });
    assert.equal(result.allowed, true);
...
describe('evaluateQuestionPolicy autopilot deep-interview wait', { concurrency: false }, () => {
  it('allows a deep-interview question to start while autopilot is in its deep-interview phase', { concurrency: false }, async () => {
    const cwd = await makeRepo();
    const sessionDir = join(cwd, '.nomx', 'state', 'sessions', 'sess-auto-start');
    await mkdir(sessionDir, { recursive: true });
    ...
    const allowed = await evaluateQuestionPolicy({
      ...
      env: { ...process.env, NOMX_TEAM_WORKER: '' },
    });
  ...
  it('blocks duplicate deep-interview questions while autopilot is already waiting', { concurrency: false }, async () => {
    const cwd = await makeRepo();
    const sessionDir = join(cwd, '.nomx', 'state', 'sessions', 'sess-auto');
    await mkdir(sessionDir, { recursive: true });
    await writeFile(join(sessionDir, 'autopilot-state.json'), JSON.stringify({
      mode: 'autopilot',
      active: true,
    ...
    await writeFile(join(cwd, '.nomx', 'state', 'session.json'), JSON.stringify({ session_id: 'sess-auto' }));
  ...
  it('rejects malformed autopilot waits that are not for the deep-interview child phase', { concurrency: false }, async () => {
    const cwd = await makeRepo();
    const sessionDir = join(cwd, '.nomx', 'state', 'sessions', 'sess-auto-ralplan-wait');
    await mkdir(sessionDir, { recursive: true });
    await writeFile(join(sessionDir, 'autopilot-state.json'), JSON.stringify({
      mode: 'autopilot',
      active: true,
