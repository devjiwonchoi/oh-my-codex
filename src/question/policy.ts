...
export async function evaluateQuestionPolicy(
  ...
  if (activeTeams.length > 0) {
    ...
    return {
      ...
      sessionId,
      code: 'team_blocked',
      message: `nomx question is unavailable while this session owns active team mode: ${summary}.`,
      fallbackAllowed: false,
      activeModes,
      activeSkills,
      activeTeams,
  ...
  if (blocked.length > 0) {
    ...
    return {
      ...
      message: `nomx question is unavailable while auto-executing workflows are active: ${blocked.join(', ')}.`,
      fallbackAllowed: false,
      activeModes,
      activeSkills,
      activeTeams,
