...
export function resolveTeamIdentityScope(env: NodeJS.ProcessEnv = process.env): TeamIdentityScope {
  const sessionId = (env.NOMX_SESSION_ID || env.CODEX_SESSION_ID || env.SESSION_ID || '').trim();
  if (sessionId) {
...
function teamLookupRoots(cwd: string, env: NodeJS.ProcessEnv = process.env): string[] {
  ...
  const explicit = typeof env.NOMX_TEAM_STATE_ROOT === 'string' ? env.NOMX_TEAM_STATE_ROOT : '';
  addStateRoot(explicit);
  addStateRoot(join(resolve(cwd), '.nomx', 'state'));
  return roots;
}
