...
const NOMX_DISPATCH_LOCK_TIMEOUT_ENV = 'NOMX_DISPATCH_LOCK_TIMEOUT_MS';
...
export function resolveDispatchLockTimeoutMs(env: NodeJS.ProcessEnv = process.env): number {
  const raw = env[NOMX_DISPATCH_LOCK_TIMEOUT_ENV];
    ...
    try {
      ...
      if (Date.now() > deadline) {
        throw new Error(
          ...
          `Set ${NOMX_DISPATCH_LOCK_TIMEOUT_ENV} to increase (current: ${timeoutMs}ms, max: ${MAX_DISPATCH_LOCK_TIMEOUT_MS}ms).`
        );
      }
