...
async function readTeamNudgeProgressMs(cwd: string, teamName: string): Promise<number> {
  const nudgeState = await readJsonIfExists(join(cwd, '.nomx', 'state', 'team-leader-nudge.json'));
...
async function readCurrentTaskBaselineMs(worktreePath: string): Promise<number> {
  return await statMsIfExists(join(worktreePath, '.nomx', 'state', 'current-task-baseline.json'));
}
