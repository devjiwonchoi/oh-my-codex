...
function buildLaunchHints(
  ...
  if (mode === 'team') {
    return {
      shellCommand: `nomx team ${recommendedHeadcount}:${fallbackRole} ${toQuotedCliArg(task)}`,
...
function buildVerificationPlan(
  ...
  if (mode === 'team') {
    ...
    return {
      ...
      checkpoints: [
        'Launch via `nomx team ...` (or `$team ...`) so the team runtime owns both parallel delivery and coordinated verification.',
