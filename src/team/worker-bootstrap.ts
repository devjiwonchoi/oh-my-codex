...
6. Use claim-safe lifecycle APIs only:
   ...
   - \`nomx team api transition-task-status --json\`
   - \`nomx team api release-task-claim --json\` only for rollback to pending
7. Use mailbox delivery flow:
   - \`nomx team api mailbox-list --input "{\"team_name\":\"${options.teamName}\",\"worker\":\"${options.workerName}\"}" --json\`
   - \`nomx team api mailbox-mark-delivered --input "{\"team_name\":\"${options.teamName}\",\"worker\":\"${options.workerName}\",\"message_id\":\"<MESSAGE_ID>\"}" --json\`
8. Preserve leader steering via inbox/mailbox nudges; task payload stays in inbox/task JSON, not this file.
9. Do not pass \`workingDirectory\` to legacy team_* MCP tools; use \`nomx team api\` CLI interop.
...
7. Request a claim via CLI interop (\`nomx team api claim-task --json\`); do not directly set lifecycle fields in the task file
...
10. On completion/failure, use lifecycle transition APIs:
   - \`nomx team api transition-task-status --json\` with from \`"in_progress"\` to \`"completed"\` or \`"failed"\`
   - Include \`result\` (for completed) or \`error\` (for failed) in the transition patch
11. Use \`nomx team api release-task-claim --json\` only for rollback/requeue to \`pending\` (not for completion)
...
15. For legacy team_* MCP tools (hard-deprecated), switch to \`nomx team api\` CLI interop; do not pass workingDirectory unless the lead explicitly tells you to

## Message Protocol
When calling \`nomx team api send-message\`, you MUST always include:
...
Example:
nomx team api send-message --input "{\"team_name\":\"${teamName}\",\"from_worker\":\"<your-worker-name>\",\"to_worker\":\"leader-fixed\",\"body\":\"ACK: <your-worker-name> initialized\"}" --json
...
1. Read: \`nomx team api mailbox-list --input "{\"team_name\":\"${teamName}\",\"worker\":\"<your-worker-name>\"}" --json\`
2. Mark delivered: \`nomx team api mailbox-mark-delivered --input "{\"team_name\":\"${teamName}\",\"worker\":\"<your-worker-name>\",\"message_id\":\"<MESSAGE_ID>\"}" --json\`
...
- Include exactly one of these lines in the task completion \`result\` passed to \`nomx team api transition-task-status\`:
...
7. Request a claim via CLI interop (\`nomx team api claim-task --json\`) to claim it
8. Complete the work described in the task
9. After completing work, commit your changes before reporting completion:
   \`git add -A && git commit -m "task: <task-subject>"\`
   This ensures your changes are available for incremental integration into the leader branch.
10. Complete/fail it via lifecycle transition API (\`nomx team api transition-task-status --json\`) from \`"in_progress"\` to \`"completed"\` or \`"failed"\` (include \`result\`/\`error\`)
11. Use \`nomx team api release-task-claim --json\` only for rollback to \`pending\`
...
14. For legacy team_* MCP tools (hard-deprecated), use \`nomx team api\`; do not pass \`workingDirectory\` unless the lead explicitly asks (if resolution fails, use leader cwd: \`${leaderCwd}\`)
...
## Message Protocol
When using \`nomx team api send-message\`, ALWAYS include from_worker with YOUR worker name:
- from_worker: "${workerName}"
- to_worker: "leader-fixed" (for leader) or "worker-N" (for peers)

Example: nomx team api send-message --input "{\"team_name\":\"${teamName}\",\"from_worker\":\"${workerName}\",\"to_worker\":\"leader-fixed\",\"body\":\"ACK: initialized\"}" --json
...
3. Request a claim via CLI interop (\`nomx team api claim-task --json\`)
4. Complete the work
5. After completing work, commit your changes before reporting completion:
   \`git add -A && git commit -m "task: <task-subject>"\`
   This ensures your changes are available for incremental integration into the leader branch.
6. Complete/fail via lifecycle transition API (\`nomx team api transition-task-status --json\`) from \`"in_progress"\` to \`"completed"\` or \`"failed"\` (include \`result\`/\`error\`)
7. Use \`nomx team api release-task-claim --json\` only for rollback to \`pending\`
