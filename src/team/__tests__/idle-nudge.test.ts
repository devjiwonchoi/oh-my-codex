...
if [[ "\$cmd" == "capture-pane" ]]; then
  if [[ "\${NOMX_FAIL_CAPTURE:-0}" == "1" ]]; then
    exit 1
  fi

  token=""
  if [[ -n "\${NOMX_CAPTURE_SEQ_FILE:-}" && -f "\${NOMX_CAPTURE_SEQ_FILE}" ]]; then
    token="\$(head -n 1 "\${NOMX_CAPTURE_SEQ_FILE}" || true)"
    if [[ -n "\$token" ]]; then
      tail -n +2 "\${NOMX_CAPTURE_SEQ_FILE}" > "\${NOMX_CAPTURE_SEQ_FILE}.tmp" || true
      mv "\${NOMX_CAPTURE_SEQ_FILE}.tmp" "\${NOMX_CAPTURE_SEQ_FILE}"
    fi
  fi
  if [[ -z "\$token" ]]; then
    token="\${NOMX_CAPTURE_TOKEN:-IDLE}"
  fi
...
if [[ "\$cmd" == "send-keys" ]]; then
  if [[ "\${NOMX_FAIL_SEND_KEYS:-0}" == "1" ]]; then
    exit 1
  fi
  exit 0
fi
...
async function withFakeTmux(run: (ctx: {
  ...
  const root = await mkdtemp(join(tmpdir(), 'nomx-idle-nudge-test-'));
  ...
  const prevCaptureSeq = process.env.NOMX_CAPTURE_SEQ_FILE;
  const prevCaptureToken = process.env.NOMX_CAPTURE_TOKEN;
  const prevFailSendKeys = process.env.NOMX_FAIL_SEND_KEYS;
  const prevFailCapture = process.env.NOMX_FAIL_CAPTURE;
  ...
  try {
    ...
    process.env.NOMX_CAPTURE_SEQ_FILE = captureSeqPath;
    process.env.NOMX_CAPTURE_TOKEN = 'IDLE';
    delete process.env.NOMX_FAIL_SEND_KEYS;
    delete process.env.NOMX_FAIL_CAPTURE;
    ...
    if (typeof prevCaptureSeq === 'string') process.env.NOMX_CAPTURE_SEQ_FILE = prevCaptureSeq;
    else delete process.env.NOMX_CAPTURE_SEQ_FILE;

    if (typeof prevCaptureToken === 'string') process.env.NOMX_CAPTURE_TOKEN = prevCaptureToken;
    else delete process.env.NOMX_CAPTURE_TOKEN;

    if (typeof prevFailSendKeys === 'string') process.env.NOMX_FAIL_SEND_KEYS = prevFailSendKeys;
    else delete process.env.NOMX_FAIL_SEND_KEYS;

    if (typeof prevFailCapture === 'string') process.env.NOMX_FAIL_CAPTURE = prevFailCapture;
    else delete process.env.NOMX_FAIL_CAPTURE;
...
describe('idle-nudge', () => {
  ...
  it('throttles scans that happen too soon after the previous scan', async () => {
    await withFakeTmux(async ({ tmuxLogPath }) => {
      await withMockedNow(10_000, async (setNow) => {
        ...
        const first = await tracker.checkAndNudge(['%2'], undefined, 'nomx-team-a');
        assert.deepEqual(first, ['%2']);
        const firstLog = await readFile(tmuxLogPath, 'utf-8');

        setNow(11_000); // < 5000ms scan interval
        const second = await tracker.checkAndNudge(['%2'], undefined, 'nomx-team-a');
  ...
  it('never nudges the leader pane', async () => {
    await withFakeTmux(async ({ tmuxLogPath }) => {
      const tracker = new NudgeTracker({ delayMs: 0, maxCount: 3, message: 'nudge' });
      const nudged = await tracker.checkAndNudge(['%1'], '%1', 'nomx-team-a');
  ...
  it('resets idle timer when pane becomes active before delay elapses', async () => {
    await withFakeTmux(async ({ setCaptureSequence }) => {
      ...
      await withMockedNow(10_000, async (setNow) => {
        ...
        const r1 = await tracker.checkAndNudge(['%2'], undefined, 'nomx-team-a');
        assert.deepEqual(r1, []);

        setNow(16_000);
        const r2 = await tracker.checkAndNudge(['%2'], undefined, 'nomx-team-a');
        assert.deepEqual(r2, []);

        setNow(22_000);
        const r3 = await tracker.checkAndNudge(['%2'], undefined, 'nomx-team-a');
        assert.deepEqual(r3, []);

        setNow(28_000);
        const r4 = await tracker.checkAndNudge(['%2'], undefined, 'nomx-team-a');
        assert.deepEqual(r4, []);

        setNow(39_000);
        const r5 = await tracker.checkAndNudge(['%2'], undefined, 'nomx-team-a');
  ...
  it('does not count nudges when sendToWorker fails', async () => {
    await withFakeTmux(async () => {
      process.env.NOMX_FAIL_SEND_KEYS = '1';

      await withMockedNow(10_000, async () => {
        const tracker = new NudgeTracker({ delayMs: 0, maxCount: 3, message: 'nudge' });
        const nudged = await tracker.checkAndNudge(['%2'], undefined, 'nomx-team-a');
        assert.deepEqual(nudged, []);
        assert.equal(tracker.totalNudges, 0);
        assert.deepEqual(tracker.getSummary(), {});
      });
  ...
  it('returns empty capture and non-idle when capture-pane command fails', async () => {
    await withFakeTmux(async () => {
      process.env.NOMX_FAIL_CAPTURE = '1';
