...
async function loadRuntimeCliModule() {
  process.env.NOMX_RUNTIME_CLI_DISABLE_AUTO_START = '1';
...
describe('runtime-cli helpers', () => {
  ...
  it('refreshes pane targets from live team config after scale changes', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-runtime-cli-live-'));
    try {
  ...
  it('reads task results from explicit NOMX_TEAM_STATE_ROOT during shutdown collection', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-runtime-cli-env-root-cwd-'));
    const explicitStateRoot = await mkdtemp(join(tmpdir(), 'nomx-runtime-cli-env-root-state-'));
  ...
  it('gracefully shuts down only when the leader explicitly requests shutdown', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-runtime-cli-shutdown-'));
    const previousTeamStateRoot = process.env.NOMX_TEAM_STATE_ROOT;
    ...
    try {
      ...
      const teamRoot = join(cwd, '.nomx', 'state', 'team', 'shutdown-fallback');
  ...
  it('does not auto-force failed-task shutdown without explicit issue confirmation', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-runtime-cli-confirm-issues-'));
    const previousTeamStateRoot = process.env.NOMX_TEAM_STATE_ROOT;
    delete process.env.NOMX_TEAM_STATE_ROOT;
    try {
      ...
      const teamRoot = join(cwd, '.nomx', 'state', 'team', 'shutdown-confirm-required');
      assert.equal(existsSync(teamRoot), true);
  ...
  it('does not auto-shutdown merely because monitorTeam reaches complete', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-runtime-cli-complete-'));
    const previousTeamStateRoot = process.env.NOMX_TEAM_STATE_ROOT;
    delete process.env.NOMX_TEAM_STATE_ROOT;
    try {
      ...
      const teamRoot = join(cwd, '.nomx', 'state', 'team', 'runtime-cli-complete');
      assert.equal(existsSync(teamRoot), true);
  ...
  it('preserves team state when building terminal output for completed teams', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-runtime-cli-preserve-complete-'));
    const previousTeamStateRoot = process.env.NOMX_TEAM_STATE_ROOT;
    try {
      delete process.env.NOMX_TEAM_STATE_ROOT;
      ...
      const teamRoot = join(cwd, '.nomx', 'state', 'team', 'runtime-cli-preserve-complete');
      assert.equal(existsSync(teamRoot), true);
  ...
  it('preserves team state when building terminal output for failed teams', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-runtime-cli-preserve-failed-'));
    const previousTeamStateRoot = process.env.NOMX_TEAM_STATE_ROOT;
    try {
      delete process.env.NOMX_TEAM_STATE_ROOT;
      ...
      const teamRoot = join(cwd, '.nomx', 'state', 'team', 'runtime-cli-preserve-failed');
      assert.equal(existsSync(teamRoot), true);
  ...
  it('reports cancelled terminal phases without deleting team state', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-runtime-cli-preserve-cancelled-'));
    const previousTeamStateRoot = process.env.NOMX_TEAM_STATE_ROOT;
    try {
      delete process.env.NOMX_TEAM_STATE_ROOT;
      ...
      const teamRoot = join(cwd, '.nomx', 'state', 'team', 'runtime-cli-preserve-cancelled');
      assert.equal(existsSync(teamRoot), true);
