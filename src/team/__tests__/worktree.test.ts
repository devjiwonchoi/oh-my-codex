...
async function initRepo(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'nomx-worktree-test-'));
...
describe('worktree planning', () => {
  it('plans dedicated autoresearch branch and path naming', async () => {
    ...
    try {
      ...
      assert.match(planned.worktreePath.replace(/\\/g, '/'), /\.nomx\/worktrees\/autoresearch-demo-mission-20260314t000000z$/);
    } finally {
