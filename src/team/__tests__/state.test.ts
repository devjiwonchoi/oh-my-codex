...
describe('team state', () => {
  it('initTeamState creates correct directory structure and config.json', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-state-'));
    try {
      const cfg = await initTeamState('team-1', 'do stuff', 'executor', 2, cwd);

      const root = join(cwd, '.nomx', 'state', 'team', 'team-1');
      ...
      assert.equal(diskCfg.tmux_session, 'nomx-team-team-1');
  ...
  it('initTeamState persists the default lifecycle profile', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-lifecycle-profile-'));
    try {
  ...
  it('migrateV1ToV2 writes manifest.v2.json idempotently from legacy config.json', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-migrate-'));
    try {
      await initTeamState('team-mig', 't', 'executor', 1, cwd);

      // Simulate a legacy team by removing v2 manifest.
      const root = join(cwd, '.nomx', 'state', 'team', 'team-mig');
  ...
  it('backfills missing or blank tmux pane owner ids in legacy manifests', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-pane-owner-backfill-'));
    try {
      await initTeamState('team-pane-owner-backfill', 't', 'executor', 1, cwd);
      const manifestPath = join(cwd, '.nomx', 'state', 'team', 'team-pane-owner-backfill', 'manifest.v2.json');
  ...
  it('normalizes legacy manifest policy with dispatch defaults, timeout bounds, and governance split', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-manifest-policy-'));
    try {
      await initTeamState('team-policy', 't', 'executor', 1, cwd);
      const manifestPath = join(cwd, '.nomx', 'state', 'team', 'team-policy', 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf8')) as Record<string, unknown>;
      ...
      const freshCwd = await mkdtemp(join(tmpdir(), 'nomx-team-manifest-policy-default-'));
      try {
        ...
        const freshManifestPath = join(freshCwd, '.nomx', 'state', 'team', 'team-policy-default', 'manifest.v2.json');
  ...
  it('dispatch request store enqueues, dedupes, and transitions idempotently', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-dispatch-store-'));
    try {
  ...
  it('dispatch request store keeps failed requests failed while allowing reason patches', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-dispatch-store-failed-'));
    try {
  ...
  it('initTeamState persists workspace metadata to config + manifest', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-metadata-'));
    try {
      const cfg = await initTeamState(
        'team-meta',
        't',
      ...
      assert.equal(manifest?.team_state_root, '/tmp/leader/.nomx/state');
  ...
  it('resolves task/mailbox/approval paths under explicit NOMX_TEAM_STATE_ROOT from a worker cwd (worker-env contamination regression)', async () => {
    const root = await mkdtemp(join(tmpdir(), 'nomx-team-explicit-root-'));
    const leaderCwd = join(root, 'leader');
    const workerCwd = join(root, 'worker-worktree');
    const explicitStateRoot = join(leaderCwd, '.nomx', 'state');
    const prevRoot = process.env.NOMX_TEAM_STATE_ROOT;
    try {
      await mkdir(leaderCwd, { recursive: true });
      await mkdir(workerCwd, { recursive: true });
      await initTeamState('team-explicit-root', 't', 'executor', 1, leaderCwd);
      process.env.NOMX_TEAM_STATE_ROOT = explicitStateRoot;
      ...
      assert.equal(existsSync(join(workerCwd, '.nomx', 'state', 'team', 'team-explicit-root')), false);
    } finally {
      if (typeof prevRoot === 'string') process.env.NOMX_TEAM_STATE_ROOT = prevRoot;
      else delete process.env.NOMX_TEAM_STATE_ROOT;
  ...
  it('claimTask enforces dependency readiness (blocked_dependency)', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-claim-'));
    try {
  ...
  it('claimTask rejects in-progress claim takeover when expectedVersion is null (issue-172)', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-claim-inprogress-'));
    try {
  ...
  it('claimTask rejects in-progress claim takeover even with a matching version', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-claim-inprogress-ver-'));
    try {
  ...
  it('claimTask claim locking yields deterministic claim_conflict', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-claim-lock-'));
    try {
  ...
  it('claimTask recovers a stale task claim lock and proceeds', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-claim-stale-lock-'));
    try {
      ...
      const staleLockDir = join(cwd, '.nomx', 'state', 'team', 'team-stale-lock', 'claims', `task-${t.id}.lock`);
  ...
  it('claimTask owner write failure cleans up claim lock without orphan lock dir', { concurrency: false }, async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-claim-owner-write-fail-'));
    ...
    try {
      ...
      const lockDir = join(cwd, '.nomx', 'state', 'team', 'team-owner-write-fail', 'claims', `task-${t.id}.lock`);
  ...
  it('claimTask rejects a pending task with residual owner/claim metadata', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-claim-residual-claim-'));
    try {
      ...
      const taskPath = join(cwd, '.nomx', 'state', 'team', 'team-claim-residual', 'tasks', `task-${t.id}.json`);
  ...
  it('claimTask allows a worker to claim its own pre-assigned pending task', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-claim-assigned-owner-'));
    try {
  ...
  it('claimTask rejects pending task pre-assigned to a different worker', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-claim-owner-mismatch-'));
    try {
  ...
  it('transitionTaskStatus returns invalid_transition for illegal transition', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-transition-'));
    try {
  ...
  it('transitionTaskStatus rejects non-terminal transitions from in_progress', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-transition-nonterminal-'));
    try {
  ...
  it('transitionTaskStatus returns claim_conflict when claim owner diverges from task owner', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-transition-owner-diverge-'));
    try {
      ...
      const taskPath = join(cwd, '.nomx', 'state', 'team', 'team-trans-owner-diverge', 'tasks', `task-${t.id}.json`);
      const current = JSON.parse(await readFile(taskPath, 'utf-8')) as any;
  ...
  it('transitionTaskStatus appends task_completed event when task completes', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-events-'));
    try {
  ...
  it('transitionTaskStatus rejects broad delegated completion without spawn evidence or skip reason', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-delegation-evidence-'));
    try {
  ...
  it('transitionTaskStatus requires evidence when optional delegation carries required parallel probe', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-optional-required-probe-'));
    try {
  ...
  it('transitionTaskStatus accepts documented skip reason for broad delegated completion', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-delegation-skip-'));
    try {
  ...
  it('transitionTaskStatus rejects coordinated completion without boundary evidence', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-coordination-evidence-'));
    try {
  ...
  it('transitionTaskStatus accepts documented no-boundary rationale for coordinated tasks', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-coordination-no-boundary-'));
    try {
  ...
  it('transitionTaskStatus persists terminal result and error payloads', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-transition-payload-'));
    try {
  ...
  it('transitionTaskStatus appends task_failed event (not worker_stopped) when task fails', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-failed-'));
    try {
      ...
      const eventsPath = join(cwd, '.nomx', 'state', 'team', 'team-failed', 'events', 'events.ndjson');
      const content = await readFile(eventsPath, 'utf-8');
  ...
  it('releaseTaskClaim reverts a claimed task back to pending under claim lock', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-release-'));
    try {
  ...
  it('releaseTaskClaim returns claim_conflict when claim token changed, even for the owner', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-release-owner-'));
    try {
      ...
      const taskPath = join(cwd, '.nomx', 'state', 'team', 'team-release-owner', 'tasks', `task-${t.id}.json`);
      const current = JSON.parse(await readFile(taskPath, 'utf-8')) as any;
  ...
  it('releaseTaskClaim on a completed task returns already_terminal and does not reopen it', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-release-terminal-'));
    try {
  ...
  it('transitionTaskStatus returns lease_expired when claim lease has passed', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-lease-trans-'));
    try {
      ...
      const taskPath = join(cwd, '.nomx', 'state', 'team', 'team-lease-trans', 'tasks', `task-${t.id}.json`);
      const current = JSON.parse(await readFile(taskPath, 'utf-8')) as any;
  ...
  it('releaseTaskClaim on a failed task returns already_terminal and does not reopen it', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-release-failed-'));
    try {
  ...
  it('releaseTaskClaim returns lease_expired when lease has expired and caller is not the owner', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-lease-release-'));
    try {
      ...
      const taskPath = join(cwd, '.nomx', 'state', 'team', 'team-lease-release', 'tasks', `task-${t.id}.json`);
      const current = JSON.parse(await readFile(taskPath, 'utf-8')) as any;
      current.claim.leased_until = new Date(Date.now() - 1000).toISOString();
      await writeFile(taskPath, JSON.stringify(current, null, 2));
  ...
  it('releaseTaskClaim returns lease_expired when lease has expired, even for the owner', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-lease-release-owner-'));
    try {
      ...
      const taskPath = join(cwd, '.nomx', 'state', 'team', 'team-lease-release-owner', 'tasks', `task-${t.id}.json`);
      const current = JSON.parse(await readFile(taskPath, 'utf-8')) as any;
      current.claim.leased_until = new Date(Date.now() - 1000).toISOString();
      await writeFile(taskPath, JSON.stringify(current, null, 2));
  ...
  it('reclaimExpiredTaskClaim reopens an expired in-progress task so another worker can claim it', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-reclaim-expired-'));
    try {
      ...
      const taskPath = join(cwd, '.nomx', 'state', 'team', 'team-reclaim-expired', 'tasks', `task-${t.id}.json`);
      const current = JSON.parse(await readFile(taskPath, 'utf-8')) as any;
      current.claim.leased_until = new Date(Date.now() - 1000).toISOString();
      await writeFile(taskPath, JSON.stringify(current, null, 2));
  ...
  it('mailbox APIs: DM, broadcast, and mark delivered', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-mailbox-'));
    try {
      ...
      const mailboxDisk = await readFile(join(cwd, '.nomx', 'state', 'team', 'team-msg', 'mailbox', 'worker-2.json'), 'utf8');
  ...
  it('sendDirectMessage recreates mailbox directory when missing', async () => {
    ...
    try {
      ...
      await rm(join(cwd, '.nomx', 'state', 'team', 'team-msg-recreate-mailbox', 'mailbox'), {
        recursive: true,
        force: true,
      });
      ...
      assert.equal(
        existsSync(
          join(cwd, '.nomx', 'state', 'team', 'team-msg-recreate-mailbox', 'mailbox', 'worker-2.json'),
        ),
        true,
      );
    } finally {
  ...
  it('sendDirectMessage throws team not found after team cleanup', async () => {
    ...
    try {
      ...
      await rm(join(cwd, '.nomx', 'state', 'team', 'team-msg-missing-team'), {
        recursive: true,
        force: true,
      });
      await assert.rejects(
  ...
  it('writeTaskApproval writes record and emits approval_decision event', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-approval-'));
    try {
      ...
      const eventsPath = join(cwd, '.nomx', 'state', 'team', 'team-approval-record', 'events', 'events.ndjson');
      const content = await readFile(eventsPath, 'utf-8');
  ...
  it('createTask does not overwrite existing tasks when config next_task_id is missing (legacy)', async () => {
    ...
    try {
      ...
      const configPath = join(cwd, '.nomx', 'state', 'team', 'team-legacy', 'config.json');
  ...
  it('createTask does not overwrite existing tasks when manifest/config next_task_id lags disk', async () => {
    ...
    try {
      ...
      const teamRoot = join(cwd, '.nomx', 'state', 'team', 'team-stale-next-id');
  ...
  it('listTasks reads task files in parallel', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-list-parallel-'));
    try {
  ...
  it('listTasks ignores malformed and id-mismatched task payloads', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-list-validate-'));
    try {
      ...
      await writeFile(
        join(cwd, '.nomx', 'state', 'team', 'team-list-validate', 'tasks', 'task-2.json'),
        JSON.stringify({
          id: '999',
          subject: 'mismatch',
          description: 'bad',
      ...
      await writeFile(
        join(cwd, '.nomx', 'state', 'team', 'team-list-validate', 'tasks', 'task-3.json'),
  ...
  it('readTask returns null for malformed JSON', async () => {
    ...
    try {
      ...
      const badPath = join(cwd, '.nomx', 'state', 'team', 'team-6', 'tasks', 'task-1.json');
  ...
  it('writeWorkerInbox writes content to the correct path', async () => {
    ...
    try {
      ...
      const inboxPath = join(cwd, '.nomx', 'state', 'team', 'team-10', 'workers', 'worker-1', 'inbox.md');
  ...
  it('cleanupTeamState removes the directory', async () => {
    ...
    try {
      ...
      const root = join(cwd, '.nomx', 'state', 'team', 'team-12');
      assert.equal(existsSync(root), true);
  ...
  it('cleanupTeamState rejects unsafe team names before path construction', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-state-unsafe-'));
    try {
  ...
  it('initTeamState rejects invalid NOMX_TEAM_WORKER_LAUNCH_MODE values', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-state-'));
    try {
      await assert.rejects(
        () => initTeamState(
          ...
          {
            ...process.env,
            NOMX_TEAM_WORKER_LAUNCH_MODE: 'tmux',
          },
        ),
  ...
  it('claimTask returns task_not_found for non-existent task id', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-claim-missing-'));
    try {
  ...
  it('resolveDispatchLockTimeoutMs returns default when env not set', () => {
    ...
    assert.equal(resolveDispatchLockTimeoutMs({ NOMX_DISPATCH_LOCK_TIMEOUT_MS: '' }), 15_000);
    assert.equal(resolveDispatchLockTimeoutMs({ NOMX_DISPATCH_LOCK_TIMEOUT_MS: 'not-a-number' }), 15_000);
  });

  it('resolveDispatchLockTimeoutMs reads from env and clamps to bounds', () => {
    // Reads value from env
    assert.equal(resolveDispatchLockTimeoutMs({ NOMX_DISPATCH_LOCK_TIMEOUT_MS: '30000' }), 30_000);
    // Clamps to minimum
    assert.equal(resolveDispatchLockTimeoutMs({ NOMX_DISPATCH_LOCK_TIMEOUT_MS: '0' }), 1_000);
    assert.equal(resolveDispatchLockTimeoutMs({ NOMX_DISPATCH_LOCK_TIMEOUT_MS: '-500' }), 1_000);
    // Clamps to maximum
    assert.equal(resolveDispatchLockTimeoutMs({ NOMX_DISPATCH_LOCK_TIMEOUT_MS: '999999' }), 120_000);
    // Floors non-integer
    assert.equal(resolveDispatchLockTimeoutMs({ NOMX_DISPATCH_LOCK_TIMEOUT_MS: '5000.9' }), 5_000);
  });

  it('dispatch lock error message includes timeout hint', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-dispatch-lock-timeout-'));
    try {
      await initTeamState('team-lock-hint', 'task', 'executor', 1, cwd);
      // Hold the lock by creating the lock directory manually
      const lockDir = join(cwd, '.nomx', 'state', 'team', 'team-lock-hint', 'dispatch', '.lock');
      await mkdir(lockDir, { recursive: true });

      // Use a very short timeout via env override so the test is fast
      const origEnv = process.env.NOMX_DISPATCH_LOCK_TIMEOUT_MS;
      process.env.NOMX_DISPATCH_LOCK_TIMEOUT_MS = '1000';
      try {
        await assert.rejects(
          () => enqueueDispatchRequest('team-lock-hint', { kind: 'inbox', to_worker: 'worker-1', trigger_message: 'test' }, cwd),
          (err: Error) => {
            assert.ok(err.message.includes('NOMX_DISPATCH_LOCK_TIMEOUT_MS'), `Expected hint in error, got: ${err.message}`);
            return true;
          }
        );
      } finally {
        if (origEnv === undefined) {
          delete process.env.NOMX_DISPATCH_LOCK_TIMEOUT_MS;
        } else {
          process.env.NOMX_DISPATCH_LOCK_TIMEOUT_MS = origEnv;
        }
  ...
  it('sanitizes persisted integration snapshot statuses to the contract', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-monitor-contract-'));
    try {
      await initTeamState('team-monitor-contract', 't', 'executor', 1, cwd);
      const monitorPath = join(cwd, '.nomx', 'state', 'team', 'team-monitor-contract', 'monitor-snapshot.json');
