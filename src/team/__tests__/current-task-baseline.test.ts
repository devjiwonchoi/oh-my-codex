...
async function initRepo(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'nomx-current-task-baseline-'));
...
describe('current-task-baseline', () => {
  it('records a launch worktree branch baseline on successful ensureWorktree', async () => {
    ...
    try {
      ...
      assert.equal(existsSync(join(repo, '.nomx', 'state', 'current-task-baseline.json')), true);
    } finally {
  ...
  it('updates branch lifecycle metadata when PR info is observed', async () => {
    ...
    try {
      ...
      upsertCurrentTaskBaseline(repo, {
        ...
        issue_number: 1407,
        pr_number: 1416,
        pr_url: 'https://github.com/Yeachan-Heo/nomx/pull/1416',
        status: 'merged',
      });

      const raw = JSON.parse(readFileSync(join(repo, '.nomx', 'state', 'current-task-baseline.json'), 'utf-8'));
