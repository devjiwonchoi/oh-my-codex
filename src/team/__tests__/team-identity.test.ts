...
async function writePhase(cwd: string, teamName: string, currentPhase: string, updatedAt: string): Promise<void> {
  await writeFile(join(cwd, '.nomx', 'state', 'team', teamName, 'phase.json'), JSON.stringify({
...
describe('team identity', () => {
  ...
  it('resolves display names from NOMX_TEAM_STATE_ROOT when cwd has no local team state', async () => {
    const leaderCwd = await mkdtemp(join(tmpdir(), 'nomx-team-identity-leader-'));
    const workerCwd = await mkdtemp(join(tmpdir(), 'nomx-team-identity-worker-'));
    try {
      await initTeamState('shared-demo-aaaaaaaa', 'task', 'executor', 1, leaderCwd, undefined, { NOMX_SESSION_ID: 'session-shared' }, {
      ...
      assert.equal(
        resolveTeamNameForCurrentContext('shared-demo', workerCwd, {
          NOMX_SESSION_ID: 'session-shared',
          NOMX_TEAM_STATE_ROOT: join(leaderCwd, '.nomx', 'state'),
        }),
  ...
  it('prefers active display-name candidates over retained terminal states', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-identity-active-'));
    try {
      await initTeamState('demo-active', 'task', 'executor', 1, cwd, undefined, { NOMX_SESSION_ID: 'session-active' }, {
        display_name: 'demo', requested_name: 'demo', identity_source: 'env-session',
      });
      await initTeamState('demo-terminal', 'task', 'executor', 1, cwd, undefined, { NOMX_SESSION_ID: 'session-terminal' }, {
        display_name: 'demo', requested_name: 'demo', identity_source: 'env-session',
      });
  ...
  it('prefers active display-name candidates over an exact retained terminal directory', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-identity-exact-terminal-'));
    try {
      await initTeamState('demo', 'task', 'executor', 1, cwd, undefined, { NOMX_SESSION_ID: 'session-terminal' }, {
        display_name: 'demo', requested_name: 'demo', identity_source: 'env-session',
      });
      await initTeamState('demo-aaaaaaaa', 'task', 'executor', 1, cwd, undefined, { NOMX_SESSION_ID: 'session-active' }, {
        display_name: 'demo', requested_name: 'demo', identity_source: 'env-session',
      });
  ...
  it('uses current leader identity to break active display-name ties', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-identity-current-'));
    try {
      await initTeamState('demo-aaaaaaaa', 'task', 'executor', 1, cwd, undefined, { NOMX_SESSION_ID: 'session-a' }, {
        display_name: 'demo', requested_name: 'demo', identity_source: 'env-session',
      });
      await initTeamState('demo-bbbbbbbb', 'task', 'executor', 1, cwd, undefined, { NOMX_SESSION_ID: 'session-b' }, {
        display_name: 'demo', requested_name: 'demo', identity_source: 'env-session',
      });

      assert.equal(resolveTeamNameForCurrentContext('demo', cwd, { NOMX_SESSION_ID: 'session-b' }), 'demo-bbbbbbbb');
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('uses current leader identity before latest retained terminal state', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-identity-terminal-current-'));
    try {
      await initTeamState('demo-old-current', 'task', 'executor', 1, cwd, undefined, { NOMX_SESSION_ID: 'session-current' }, {
        display_name: 'demo', requested_name: 'demo', identity_source: 'env-session',
      });
      await initTeamState('demo-new-other', 'task', 'executor', 1, cwd, undefined, { NOMX_SESSION_ID: 'session-other' }, {
        display_name: 'demo', requested_name: 'demo', identity_source: 'env-session',
      });
      ...
      assert.equal(resolveTeamNameForCurrentContext('demo', cwd, { NOMX_SESSION_ID: 'session-current' }), 'demo-old-current');
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('resolves the latest retained terminal display-name state only when unambiguous', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-identity-latest-terminal-'));
    try {
      await initTeamState('demo-old', 'task', 'executor', 1, cwd, undefined, { NOMX_SESSION_ID: 'session-old' }, {
        display_name: 'demo', requested_name: 'demo', identity_source: 'env-session',
      });
      await initTeamState('demo-new', 'task', 'executor', 1, cwd, undefined, { NOMX_SESSION_ID: 'session-new' }, {
        display_name: 'demo', requested_name: 'demo', identity_source: 'env-session',
      });
  ...
  it('sanitizes unsafe lookup input instead of returning raw path-like names', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-identity-unsafe-'));
    try {
  ...
  it('resolves display names to the current session candidate and fails closed on ambiguity', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-identity-'));
    try {
      await initTeamState('demo-aaaaaaaa', 'task', 'executor', 1, cwd, undefined, { NOMX_SESSION_ID: 'session-a' }, {
        display_name: 'demo', requested_name: 'demo', identity_source: 'env-session',
      });
      ...
      assert.equal(resolveTeamNameForCurrentContext('demo', cwd, { NOMX_SESSION_ID: 'session-a' }), 'demo-aaaaaaaa');
