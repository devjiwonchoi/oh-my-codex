...
async function initRepo(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'nomx-cross-rebase-smoke-repo-'));
...
describe('cross-rebase smoke regression', () => {
  it('surfaces cross-rebase applied as an audit event without making it wakeable', async () => {
    ...
    let worker2Path = '';
    try {
      worker1Path = await addWorktree(repo, 'wk1-cross-applied', 'nomx-cross-applied-w1-');
      worker2Path = await addWorktree(repo, 'wk2-cross-applied', 'nomx-cross-applied-w2-');
  ...
  it('surfaces cross-rebase conflicts as wakeable and preserves cleanup/report evidence', async () => {
    ...
    try {
      ...
      worker1Path = await addWorktree(repo, 'wk1-cross-conflict', 'nomx-cross-conflict-w1-');
      worker2Path = await addWorktree(repo, 'wk2-cross-conflict', 'nomx-cross-conflict-w2-');
      ...
      const reportPath = join(repo, '.nomx', 'state', 'team', 'cross-rebase-conflict', 'integration-report.md');
  ...
  it('keeps cross-rebase skipped as audit-only when worker state is not eligible', async () => {
    ...
    let worker2Path = '';
    try {
      worker1Path = await addWorktree(repo, 'wk1-cross-skipped', 'nomx-cross-skipped-w1-');
      worker2Path = await addWorktree(repo, 'wk2-cross-skipped', 'nomx-cross-skipped-w2-');
