...
async function initRepo(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'nomx-shutdown-fallback-'));
...
describe('shutdown fallback worktree reports', () => {
  it('shutdownTeam checkpoints dirty detached worker worktrees, merges them, and writes a report', async () => {
    ...
    const binDir = await mkdtemp(join(tmpdir(), 'nomx-shutdown-fallback-bin-'));
...
`,
    ...
    try {
      ...
      const reportPath = join(preservedWorktreePath as string, '.nomx', 'diff.md');
      ...
      const commitHygieneJsonPath = join(repo, '.nomx', 'reports', 'team-commit-hygiene', 'team-shutdown-fallback-report.context.json');
      const commitHygieneMarkdownPath = join(repo, '.nomx', 'reports', 'team-commit-hygiene', 'team-shutdown-fallback-report.md');
