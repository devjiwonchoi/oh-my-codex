...
async function setupTeam(name: string): Promise<{ cwd: string; cleanup: () => Promise<void> }> {
  const cwd = await mkdtemp(join(tmpdir(), `nomx-interop-${name}-`));
...
async function setupDisplayTeam(
  ...
  const cwd = await mkdtemp(join(tmpdir(), `nomx-interop-${internalName}-`));
  const previousTeamStateRoot = process.env.NOMX_TEAM_STATE_ROOT;
...
async function withMailboxCompatHoleRuntime<T>(cwd: string, fn: () => Promise<T>): Promise<T> {
  ...
  const runtimePath = join(fakeBin, 'nomx-runtime');
  const previousPath = process.env.PATH;
  const previousBinary = process.env.NOMX_RUNTIME_BINARY;
  const previousBridge = process.env.NOMX_RUNTIME_BRIDGE;
...
`,
  ...
  process.env.NOMX_RUNTIME_BINARY = runtimePath;
  process.env.NOMX_RUNTIME_BRIDGE = '1';
  try {
    return await fn();
  } finally {
    if (typeof previousPath === 'string') process.env.PATH = previousPath;
    else delete process.env.PATH;
    if (typeof previousBinary === 'string') process.env.NOMX_RUNTIME_BINARY = previousBinary;
    else delete process.env.NOMX_RUNTIME_BINARY;
    if (typeof previousBridge === 'string') process.env.NOMX_RUNTIME_BRIDGE = previousBridge;
...
describe('validateCommonFields', () => {
  ...
  it('resolves a long display team_name before applying internal key validation', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-api-long-display-'));
    try {
      await initTeamState('long-display-aaaaaaaa', 'test task', 'executor', 1, cwd, undefined, { NOMX_SESSION_ID: 'session-long' }, {
      ...
      const previousSessionId = process.env.NOMX_SESSION_ID;
      try {
        process.env.NOMX_SESSION_ID = 'session-long';
        ...
        if (previousSessionId === undefined) delete process.env.NOMX_SESSION_ID;
        else process.env.NOMX_SESSION_ID = previousSessionId;
      }
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
...
describe('executeTeamApiOperation: send-message', () => {
  ...
  it('returns the persisted leader mailbox message when hook-targeted sends are deduped from a worker worktree', async () => {
    ...
    const repoCwd = await mkdtemp(join(tmpdir(), 'nomx-interop-send-root-'));
    const workerCwd = await mkdtemp(join(tmpdir(), 'nomx-interop-send-worktree-'));
    ...
    try {
      ...
      const configPath = join(repoCwd, '.nomx', 'state', 'team', teamName, 'config.json');
      ...
      const manifestPath = join(repoCwd, '.nomx', 'state', 'team', teamName, 'manifest.v2.json');
      ...
      process.env.NOMX_TEAM_STATE_ROOT = join(repoCwd, '.nomx', 'state');
      ...
      const mailbox = JSON.parse(await readFile(
        join(repoCwd, '.nomx', 'state', 'team', teamName, 'mailbox', 'leader-fixed.json'),
        'utf-8',
  ...
  it('returns the persisted mailbox message when bridge compat omits the mailbox row under an explicit shared state root', async () => {
    ...
    const root = await mkdtemp(join(tmpdir(), 'nomx-interop-shared-root-'));
...
describe('executeTeamApiOperation: mailbox-mark-delivered', () => {
  it('marks a message delivered after sending and promotes matching dispatch receipt', async () => {
    ...
    try {
      ...
      await mkdir(join(cwd, '.nomx', 'state', 'team', 'mark-dlv', 'mailbox', 'worker-2'), { recursive: true });
  ...
  it('marks leader-fixed mailbox delivery from a worker worktree and resolves the matching dispatch receipt', async () => {
    ...
    const repoCwd = await mkdtemp(join(tmpdir(), 'nomx-interop-mark-dlv-root-'));
    const workerCwd = await mkdtemp(join(tmpdir(), 'nomx-interop-mark-dlv-worktree-'));
    const prevTeamStateRoot = process.env.NOMX_TEAM_STATE_ROOT;
    delete process.env.NOMX_TEAM_STATE_ROOT;
...
describe('executeTeamApiOperation: mailbox-mark-notified', () => {
  it('marks a message notified after sending', async () => {
    ...
    try {
      // Ensure the worker-2 mailbox directory exists so sendDirectMessage can write
      await mkdir(join(cwd, '.nomx', 'state', 'team', 'mark-ntf', 'mailbox', 'worker-2'), { recursive: true });
      const sendResult = await executeTeamApiOperation('send-message', {
...
describe('executeTeamApiOperation: list-tasks', () => {
  ...
  it('resolves team working directory from manifest metadata over worker identity/config fallbacks', async () => {
    ...
    const cwdA = await mkdtemp(join(tmpdir(), 'nomx-interop-meta-a-'));
    const cwdB = await mkdtemp(join(tmpdir(), 'nomx-interop-meta-b-'));
    ...
    try {
      ...
      const teamRootA = join(cwdA, '.nomx', 'state', 'team', teamName);
      ...
      config.team_state_root = join(cwdA, '.nomx', 'state');
      manifest.team_state_root = join(cwdB, '.nomx', 'state');
  ...
  it('resolves display team names from NOMX_TEAM_STATE_ROOT when API calls run outside the leader cwd', async () => {
    const { cwd: leaderCwd, cleanup } = await setupDisplayTeam('api-root-display-11111111', 'api-root-display', 'session-api-root-display');
    const workerCwd = await mkdtemp(join(tmpdir(), 'nomx-interop-api-root-worker-'));
    try {
      process.env.NOMX_TEAM_STATE_ROOT = join(leaderCwd, '.nomx', 'state');
  ...
  it('prefers NOMX_TEAM_STATE_ROOT over manifest metadata when resolving the team working directory', async () => {
    const teamName = 'list-tsk-env-root';
    const cwdA = await mkdtemp(join(tmpdir(), 'nomx-interop-env-a-'));
    const cwdB = await mkdtemp(join(tmpdir(), 'nomx-interop-env-b-'));
    const prevTeamWorker = process.env.NOMX_TEAM_WORKER;
    const prevTeamStateRoot = process.env.NOMX_TEAM_STATE_ROOT;
    delete process.env.NOMX_TEAM_STATE_ROOT;
    process.env.NOMX_TEAM_WORKER = `${teamName}/worker-1`;
    ...
    try {
      ...
      process.env.NOMX_TEAM_STATE_ROOT = join(cwdA, '.nomx', 'state');
...
describe('executeTeamApiOperation: read-config', () => {
  ...
  it('returns team_not_found for nonexistent team', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-interop-cfg-nf-'));
    try {
      const result = await executeTeamApiOperation('read-config', {
...
describe('executeTeamApiOperation: read-stall-state', () => {
  ...
  it('reads leader runtime activity from boxed NOMX_ROOT state consistently with writer path', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-interop-boxed-cwd-'));
    const box = await mkdtemp(join(tmpdir(), 'nomx-interop-boxed-root-'));
    ...
    try {
      ...
      await mkdir(join(box, '.nomx', 'state'), { recursive: true });
      await writeFile(join(box, '.nomx', 'state', 'leader-runtime-activity.json'), JSON.stringify({
  ...
  it('suppresses stale native-stop attention after newer leader activity', async () => {
    ...
    try {
      ...
      await writeFile(join(cwd, '.nomx', 'state', 'leader-runtime-activity.json'), JSON.stringify({
        last_activity_at: '2026-03-10T10:06:00.000Z',
        last_source: 'team_status',
  ...
  it('suppresses stale native-stop attention after newer detached worktree progress', async () => {
    ...
    try {
      ...
      await writeFile(join(workerWorktree, '.nomx', 'state', 'current-task-baseline.json'), JSON.stringify({
        version: 1,
        tasks: [],
      }, null, 2));
...
describe('executeTeamApiOperation: get-summary', () => {
  ...
  it('returns team_not_found for nonexistent team', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-interop-sum-nf-'));
    try {
      const result = await executeTeamApiOperation('get-summary', {
...
describe('executeTeamApiOperation: error handling', () => {
  it('wraps thrown errors in an error envelope', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-interop-err-'));
    try {
      await mkdir(join(cwd, '.nomx', 'state', 'team', 'err-team'), { recursive: true });
      await writeFile(join(cwd, '.nomx', 'state', 'team', 'err-team', 'config.json'), '{}', 'utf8');
