...
async function initRepo(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-hardening-e2e-repo-'));
...
describe('team hardening e2e', () => {
  it('reopens an expired in-progress task and allows the next worker to complete the flow', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-hardening-e2e-'));
    try {
      ...
      const taskPath = join(cwd, '.nomx', 'state', 'team', 'team-hardening-e2e', 'tasks', `task-${task.id}.json`);
  ...
  it('tolerates malformed worker status from an external team state root', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-team-hardening-worker-state-'));
