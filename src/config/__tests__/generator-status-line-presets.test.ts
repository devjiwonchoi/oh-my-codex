 ...
 *
 * The NOMX default for [tui].status_line is now derived from a HudPreset
...
describe("buildMergedConfig with statusLinePreset", () => {
  ...
  it("emits the minimal status_line when statusLinePreset='minimal' on a fresh config", () => {
    const wd = "/tmp/nomx-preset-minimal";
  ...
  it("emits the focused status_line when statusLinePreset='focused' on a fresh config", () => {
    const wd = "/tmp/nomx-preset-focused";
  ...
  it("preserves a user-defined status_line even when a preset is requested", async () => {
    const wd = await mkdtemp(join(tmpdir(), "nomx-preset-pres-"));
    try {
  ...
  it("upgrades a previously NOMX-managed status_line to the new preset", async () => {
    const wd = await mkdtemp(join(tmpdir(), "nomx-preset-upgrade-"));
    try {
      const configPath = join(wd, "config.toml");
      ...
      // Now request minimal — the NOMX-managed status_line should be replaced
...
describe("user-customized status_line inside the NOMX marker block", () => {
  // Regression guard for the codex-flagged scenario: a user manually editing
  // a [tui] block inside the NOMX marker to a value that happens to byte-match
  ...
  it("preserves a user-edited preset-literal status_line that lacks the managed marker", async () => {
    const wd = await mkdtemp(join(tmpdir(), "nomx-preset-coll-"));
    try {
      const configPath = join(wd, "config.toml");
      const userEditedConfig = [
        ...
        "[tui]",
        // No "# nomx:managed-status-line" marker — this is a user edit that
  ...
  it("recognizes the legacy seven-field default as NOMX-managed for back-compat", async () => {
    const wd = await mkdtemp(join(tmpdir(), "nomx-preset-legacy-"));
    try {
      const configPath = join(wd, "config.toml");
      // Pre-marker install: NOMX wrote the focused 7-field array without
  ...
  it("emits the managed-status-line marker comment when NOMX writes status_line", async () => {
    const wd = await mkdtemp(join(tmpdir(), "nomx-preset-marker-"));
    try {
      const configPath = join(wd, "config.toml");
      await mergeConfig(configPath, wd, {});
      const toml = await readFile(configPath, "utf-8");
...
describe("mergeConfig with statusLinePreset (integration via .nomx/hud-config.json)", () => {
  it("renders the preset selected in MergeOptions when [tui] does not yet exist", async () => {
    const wd = await mkdtemp(join(tmpdir(), "nomx-preset-int-"));
    try {
