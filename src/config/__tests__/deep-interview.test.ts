...
describe('deep-interview runtime config', () => {
  it('resolves project .nomx/config.toml and applies profile-specific values', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-deep-interview-config-'));
    const homeDir = await mkdtemp(join(tmpdir(), 'nomx-deep-interview-home-'));
    try {
...
`,
      ...
      assert.deepEqual(config, {
        ...
        maxRounds: 15,
        enableChallengeModes: false,
        sourcePath: join(cwd, '.nomx', 'config.toml'),
      });
    } finally {
...
`,
  ...
  it('prefers project config over root nomx.toml and user config', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-deep-interview-config-'));
    const homeDir = await mkdtemp(join(tmpdir(), 'nomx-deep-interview-home-'));
    try {
      await mkdir(join(cwd, '.nomx'), { recursive: true });
      await mkdir(join(homeDir, '.nomx'), { recursive: true });
      await writeFile(join(homeDir, '.nomx', 'config.toml'), '[nomx.deepInterview]\nstandardThreshold = 0.40\n');
      await writeFile(join(cwd, 'nomx.toml'), '[nomx.deepInterview]\nstandardThreshold = 0.30\n');
      ...
      assert.equal(config?.sourcePath, join(cwd, '.nomx', 'config.toml'));
      const candidates = getDeepInterviewConfigCandidatePaths({ cwd, homeDir });
      assert.equal(candidates[0]?.path, join(cwd, '.nomx', 'config.toml'));
      assert.equal(candidates[1]?.path, join(cwd, 'nomx.toml'));
      assert.equal(
        candidates.find((candidate) => candidate.precedence === 'user')?.path,
        join(homeDir, '.nomx', 'config.toml'),
      );
    } finally {
      await rm(cwd, { recursive: true, force: true });
      await rm(homeDir, { recursive: true, force: true });
  ...
  it('resolves each documented config source when higher-precedence sources are absent', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-deep-interview-config-sources-'));
    const homeDir = await mkdtemp(join(tmpdir(), 'nomx-deep-interview-home-sources-'));
    try {
      await mkdir(join(cwd, '.nomx'), { recursive: true });
      await mkdir(join(homeDir, '.nomx'), { recursive: true });
      ...
      await rm(join(cwd, '.nomx', 'config.toml'), { force: true });
      ...
      assert.equal(config?.sourcePath, join(cwd, 'nomx.toml'));

      await rm(join(cwd, 'nomx.toml'), { force: true });
      await writeFile(join(homeDir, '.nomx', 'config.toml'), '[nomx.deepInterview]\nstandardThreshold = 0.33\n');
      config = resolveDeepInterviewRuntimeConfig({ cwd, homeDir, text: '$deep-interview' });
      assert.equal(config?.threshold, 0.33);
      assert.equal(config?.sourcePath, join(homeDir, '.nomx', 'config.toml'));
    } finally {
      await rm(cwd, { recursive: true, force: true });
      await rm(homeDir, { recursive: true, force: true });
    }
  ...
  it('resolves repository config from nested working directories', async () => {
    ...
    try {
      ...
      assert.equal(candidatePaths[0]?.path, join(nestedCwd, '.nomx', 'config.toml'));
      assert.equal(candidatePaths[2]?.path, join(cwd, '.nomx', 'config.toml'));
    } finally {
      await rm(cwd, { recursive: true, force: true });
      await rm(homeDir, { recursive: true, force: true });
    }
...
`,
  ...
  it('does not cascade to lower-precedence configs after a parse failure', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-deep-interview-config-malformed-precedence-'));
    const homeDir = await mkdtemp(join(tmpdir(), 'nomx-deep-interview-home-malformed-precedence-'));
    const originalWarn = console.warn;
    const warnings: string[] = [];
    try {
      console.warn = (message?: unknown) => {
...
`,
  ...
  it('does not cascade to lower-precedence configs when an existing higher-precedence file omits deepInterview', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nomx-deep-interview-config-no-table-precedence-'));
    const homeDir = await mkdtemp(join(tmpdir(), 'nomx-deep-interview-home-no-table-precedence-'));
    try {
      await mkdir(join(cwd, '.nomx'), { recursive: true });
      await mkdir(join(homeDir, '.nomx'), { recursive: true });
      await writeFile(join(cwd, '.nomx', 'config.toml'), '[nomx.other]\nenabled = true\n');
      await writeFile(
        join(homeDir, '.nomx', 'config.toml'),
        `[nomx.deepInterview]
defaultProfile = "deep"
...
`,
  ...
  it('parses supported profile flags only', () => {
    ...
    assert.equal(parseDeepInterviewProfileFromText('$nomx:deep-interview --deep'), 'deep');
    assert.equal(parseDeepInterviewProfileFromText('deep interview --quick'), 'quick');
