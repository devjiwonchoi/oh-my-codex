...
/**
 * Notify events lack lifecycle provenance, so they may update activity only
 * for a subagent that native lifecycle evidence already established. They
 * must never create a tracker entry or infer a leader/child relationship.
 */
export async function recordKnownSubagentActivityForSession(
  cwd: string,
  input: { sessionId: string; threadId: string; turnId?: string; timestamp?: string },
): Promise<boolean> {
  const sessionId = input.sessionId.trim();
  const threadId = input.threadId.trim();
