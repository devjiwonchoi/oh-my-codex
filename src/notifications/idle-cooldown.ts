 ...
 *
 * Config key : notifications.idleCooldownSeconds in ~/.codex/.nomx-config.json
 * Env var    : NOMX_IDLE_COOLDOWN_SECONDS  (overrides config)
 * State file : .nomx/state/idle-notif-cooldown.json
 ...
 *
 * Resolution order:
 *   1. NOMX_IDLE_COOLDOWN_SECONDS env var
 *   2. notifications.idleCooldownSeconds in ~/.codex/.nomx-config.json
 *   3. Default: 60 seconds
 */
export function getIdleNotificationCooldownSeconds(): number {
  // 1. Environment variable override
  const envVal = process.env.NOMX_IDLE_COOLDOWN_SECONDS;
  if (envVal !== undefined) {
