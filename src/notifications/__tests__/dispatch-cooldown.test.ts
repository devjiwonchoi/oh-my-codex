...
function makeTmpStateDir(): string {
  const dir = join(tmpdir(), `nomx-dispatch-cooldown-test-${Math.random().toString(36).slice(2)}`);
...
describe('getDispatchNotificationCooldownSeconds', () => {
  afterEach(() => {
    delete process.env.NOMX_DISPATCH_COOLDOWN_SECONDS;
  });
...
describe('shouldSendDispatchNotification', () => {
  ...
  it('returns false when cooldown has NOT elapsed', () => {
    process.env.NOMX_DISPATCH_COOLDOWN_SECONDS = '60';
  ...
  it('returns true when cooldown HAS elapsed (stale timestamp)', () => {
    process.env.NOMX_DISPATCH_COOLDOWN_SECONDS = '1';
