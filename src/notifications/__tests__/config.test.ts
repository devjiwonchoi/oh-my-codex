...
const ENV_KEYS = [
  ...
  'NOMX_DISCORD_MENTION',
  'NOMX_TELEGRAM_BOT_TOKEN',
  'NOMX_TELEGRAM_NOTIFIER_BOT_TOKEN',
  'NOMX_TELEGRAM_CHAT_ID',
  'NOMX_TELEGRAM_NOTIFIER_CHAT_ID',
  'NOMX_TELEGRAM_NOTIFIER_UID',
  'NOMX_SLACK_WEBHOOK_URL',
  'NOMX_SLACK_MENTION',
  'NOMX_REPLY_ENABLED',
  'NOMX_REPLY_DISCORD_USER_IDS',
  'NOMX_REPLY_POLL_INTERVAL_MS',
  'NOMX_REPLY_RATE_LIMIT',
];
...
describe('buildConfigFromEnv', () => {
  ...
  it('builds discord-bot config from env vars', () => {
    process.env.NOMX_DISCORD_NOTIFIER_BOT_TOKEN = 'test-token';
    process.env.NOMX_DISCORD_NOTIFIER_CHANNEL = '123456';
  ...
  it('rejects invalid mention in env var', () => {
    process.env.NOMX_DISCORD_NOTIFIER_BOT_TOKEN = 'test-token';
    process.env.NOMX_DISCORD_NOTIFIER_CHANNEL = '123456';
    process.env.NOMX_DISCORD_MENTION = '@everyone';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('builds discord webhook config from env var', () => {
    process.env.NOMX_DISCORD_WEBHOOK_URL = 'https://discord.com/api/webhooks/test';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('builds slack config from env var', () => {
    process.env.NOMX_SLACK_WEBHOOK_URL = 'https://hooks.slack.com/services/test';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('includes a validated slack mention in slack config', () => {
    process.env.NOMX_SLACK_WEBHOOK_URL = 'https://hooks.slack.com/services/test';
    process.env.NOMX_SLACK_MENTION = '<!here>';
  ...
  it('uses NOMX_TELEGRAM_NOTIFIER_BOT_TOKEN as fallback', () => {
    process.env.NOMX_TELEGRAM_NOTIFIER_BOT_TOKEN = '123:fallback';
    process.env.NOMX_TELEGRAM_CHAT_ID = '999';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('uses NOMX_TELEGRAM_NOTIFIER_UID as fallback for chat ID', () => {
    process.env.NOMX_TELEGRAM_BOT_TOKEN = '123:abc';
    process.env.NOMX_TELEGRAM_NOTIFIER_UID = 'uid-999';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('builds config with multiple platforms from env', () => {
    process.env.NOMX_DISCORD_NOTIFIER_BOT_TOKEN = 'bot-token';
    process.env.NOMX_DISCORD_NOTIFIER_CHANNEL = 'channel-123';
    process.env.NOMX_TELEGRAM_BOT_TOKEN = '456:tg';
    process.env.NOMX_TELEGRAM_CHAT_ID = 'chat-789';
    process.env.NOMX_SLACK_WEBHOOK_URL = 'https://hooks.slack.com/services/test';
