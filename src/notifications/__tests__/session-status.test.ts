...
describe('session-status helper', () => {
  ...
  it('renders a bounded running summary with active subagent details', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nomx-session-status-'));
    try {
      await writeSessionStart(wd, 'sess-1', { nativeSessionId: 'native-1' });
      await writeJson(join(wd, '.nomx', 'state', 'sessions', 'sess-1', 'skill-active-state.json'), {
        active: true,
        skill: 'ralph',
        phase: 'executing',
      ...
      await writeJson(join(wd, '.nomx', 'state', 'sessions', 'sess-1', 'run-state.json'), {
        version: 1,
        mode: 'ralph',
        active: true,
        outcome: 'continue',
  ...
  it('does not report prompt-seeded ralph skill-active state as running without a live run-state', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nomx-session-status-prompt-seeded-'));
    try {
      await writeSessionStart(wd, 'sess-seeded', { nativeSessionId: 'native-seeded' });
      await writeJson(join(wd, '.nomx', 'state', 'sessions', 'sess-seeded', 'skill-active-state.json'), {
        active: true,
        skill: 'ralph',
        phase: 'planning',
        updated_at: '2026-03-20T00:04:30.000Z',
  ...
  it('renders stale partial summaries with explicit unknown fields', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nomx-session-status-stale-'));
    try {
      await mkdir(join(wd, '.nomx', 'logs'), { recursive: true });
      await writeFile(
        join(wd, '.nomx', 'logs', 'session-history.jsonl'),
        `${JSON.stringify({
          session_id: 'sess-old',
  ...
  it('does not report a stale tracked session as running when only raw session.json remains', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nomx-session-status-orphaned-'));
    try {
      await writeJson(join(wd, '.nomx', 'state', 'session.json'), {
  ...
  it('returns a bounded failure message when correlated state cannot be resolved', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nomx-session-status-empty-'));
    try {
      const status = await buildDiscordSessionStatusReply(createMapping(wd));
  ...
  it('filters root skill-active fallback to entries visible to the requested session', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nomx-session-status-foreign-root-'));
    try {
      await writeJson(join(wd, '.nomx', 'state', 'skill-active-state.json'), {
        active: true,
        skill: 'team',
        phase: 'running',
        updated_at: '2026-03-20T00:04:30.000Z',
  ...
  it('ignores root skill-active fallback when all active entries belong to another session', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nomx-session-status-root-only-foreign-'));
    try {
      await writeJson(join(wd, '.nomx', 'state', 'skill-active-state.json'), {
        active: true,
        skill: 'ralph',
