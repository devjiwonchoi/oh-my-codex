...
function makeTmpDir(): string {
  const dir = join(tmpdir(), `nomx-hook-cfg-test-${Math.random().toString(36).slice(2)}`);
...
describe('getHookConfig', () => {
  ...
  beforeEach(() => {
    tmpDir = makeTmpDir();
    resetHookConfigCache();
    delete process.env.NOMX_HOOK_CONFIG;
  });
  ...
  it('reads config from NOMX_HOOK_CONFIG env var path', () => {
    const path = join(tmpDir, 'hook.json');
    writeFileSync(path, JSON.stringify({ version: 1, enabled: true }));
    process.env.NOMX_HOOK_CONFIG = path;
    const result = getHookConfig();
    assert.ok(result !== null);
    assert.equal(result!.enabled, true);
  });

  it('returns null when NOMX_HOOK_CONFIG file does not exist', () => {
    process.env.NOMX_HOOK_CONFIG = join(tmpDir, 'nonexistent.json');
    assert.equal(getHookConfig(), null);
  });
