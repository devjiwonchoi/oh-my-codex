...
const NOMX_METADATA_SEGMENT_RE = /^\[NOMX(?:[#\]].*)?$/;
...
function isMetadataOnlyTmuxSegment(segment: string): boolean {
  return NOMX_METADATA_SEGMENT_RE.test(segment)
...
function isMetadataOnlyTmuxLine(line: string): boolean {
  ...
  const hasExplicitStatusSegment = segments.some((segment) => NOMX_METADATA_SEGMENT_RE.test(segment) || HUD_STATUS_SEGMENT_RE.test(segment));
...
function shouldUsePidFallback(): boolean {
  return process.env.NOMX_TMUX_PID_FALLBACK === "1";
}
