 ...
 *
 * Registry location: ~/.nomx/state/reply-session-registry.jsonl (global, not worktree-local)
...
const REGISTRY_PATH = join(homedir(), '.nomx', 'state', 'reply-session-registry.jsonl');
const REGISTRY_LOCK_PATH = join(homedir(), '.nomx', 'state', 'reply-session-registry.lock');
