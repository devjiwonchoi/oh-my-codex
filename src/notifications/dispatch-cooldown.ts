 ...
 *
 * Config key : notifications.dispatchCooldownSeconds in ~/.codex/.nomx-config.json
 * Env var    : NOMX_DISPATCH_COOLDOWN_SECONDS  (overrides config)
 * State file : .nomx/state/dispatch-notif-cooldown.json
 ...
 *
 * Resolution order:
 *   1. NOMX_DISPATCH_COOLDOWN_SECONDS env var
 *   2. notifications.dispatchCooldownSeconds in ~/.codex/.nomx-config.json
 *   3. Default: 60 seconds
 */
export function getDispatchNotificationCooldownSeconds(): number {
  // 1. Environment variable override
  const envVal = process.env.NOMX_DISPATCH_COOLDOWN_SECONDS;
  if (envVal !== undefined) {
  ...
  // 2. Config file
  try {
    const configPath = join(codexHome(), '.nomx-config.json');
