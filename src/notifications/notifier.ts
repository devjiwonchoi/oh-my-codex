...
/**
 * Load notification config from .nomx/notifications.json
 */
export async function loadNotificationConfig(projectRoot?: string): Promise<NotificationConfig | null> {
  const configPath = join(projectRoot || process.cwd(), '.nomx', 'notifications.json');
...
export function _buildDesktopArgs(
  ...
  if (platform === 'darwin') {
    ...
    const ps =
      ...
      `[Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier('nomx').Show($xml)`;
...
async function sendDiscordNotification(payload: NotificationPayload, webhookUrl: string): Promise<void> {
  ...
  const body = JSON.stringify({
    embeds: [{
      title: `[NOMX] ${payload.title}`,
      description: payload.message,
      color: colorMap[payload.type],
      footer: { text: `nomx | ${payload.mode || 'general'}` },
