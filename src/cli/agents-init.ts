...
export const AGENTS_INIT_USAGE = [
  "Usage: nomx agents-init [path] [--dry-run] [--force] [--verbose]",
  "       nomx deepinit [path] [--dry-run] [--force] [--verbose]",
  "",
...
export async function agentsInit(
  ...
  for (let index = 0; index < plannedDirs.length; index += 1) {
    ...
    const decision = await syncManagedAgentsFile(
      ...
      backupRoot,
      rootOverlayRisk
        ? "active nomx session detected for project root AGENTS.md"
        : undefined,
    );
  ...
  console.log(
    "- Preserves manual notes only for files already managed by nomx agents-init.",
  );
  console.log(
    "- Never overwrites unmanaged AGENTS.md files unless you pass --force.",
  );
  console.log(
    "- Avoids rewriting project-root AGENTS.md while an active nomx session is running.\n",
  );
