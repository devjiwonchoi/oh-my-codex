...
describe('decomposeTaskString', () => {
  ...
  it('keeps long analytic prose prompts in a single-worker lane by default', () => {
    const task = 'Analyze NOMX team mode reliability/efficiency weaknesses, focusing on orchestration progress detection, heartbeat/task-state coupling, tmux/state-plane brittleness, and verification gaps. Produce concrete findings with root cause, user impact, evidence pointers, and actionable recommendations suitable for a GitHub issue.';
    ...
    assert.match(plan.tasks[0].description, /Analyze NOMX team mode reliability\/efficiency weaknesses/i);
  });
