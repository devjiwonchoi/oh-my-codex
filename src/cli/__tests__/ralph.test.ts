...
describe('ralph launch wiring', () => {
  it('requires conditional native subagent routing guidance', () => {
    ...
    assert.match(instructions, /CODEX_THREAD_ID.*authenticated current leader identity/);
    assert.match(instructions, /--parent-thread "\$CODEX_THREAD_ID" --json/);
    assert.match(instructions, /never fake the role via a prompt label/);
    assert.match(instructions, /preserve legacy Ralph tier intent only as task-scoping guidance/);
    assert.match(instructions, /Use only `task_name`, `message`, and optional `fork_turns`/);
    assert.match(instructions, /do not invent `model`, `reasoning_effort`, `run_in_background`/);
    assert.match(instructions, /LOW for narrow lookup/);
    assert.match(instructions, /STANDARD for bounded implementation/);
    assert.match(instructions, /THOROUGH for complex or high-risk analysis/);
