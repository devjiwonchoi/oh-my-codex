...
async function runSetupWithCapturedLogs(
  ...
  try {
    for (const base of [cwd, process.env.HOME].filter((value): value is string => Boolean(value))) {
      const root = join(base, '.nomx');
      const identityPath = join(root, 'identity.json');
      if (existsSync(root) && !existsSync(identityPath)) {
        await writeFile(identityPath, `${JSON.stringify(createNomxRootMetadata(root), null, 2)}\n`);
      }
    }
    await setup({
...
describe('nomx setup AGENTS refresh behavior', () => {
  ...
  it('overwrites existing AGENTS.md in TTY after confirmation and moves the old file to a deterministic sibling backup', async () => {
    ...
    const home = join(wd, 'home');
    const restoreHome = setMockHome(home);
    const existing = '# nomx - Intelligent Multi-Agent Orchestration\n\nUser-owned guidance.\n';
    try {
      await mkdir(join(wd, '.nomx', 'state'), { recursive: true });
      await writeFile(join(wd, 'AGENTS.md'), existing);
      ...
      assert.match(agentsContent, /# nomx - Intelligent Multi-Agent Orchestration/);
  ...
  it('keeps --merge-agents idempotent for already generated AGENTS.md and refreshes stale model rows', async () => {
    ...
    try {
      ...
      assert.match(output, /Merged NOMX-managed AGENTS\.md sections into project root\./);
      assert.equal(countOccurrences(agentsContent, '<!-- nomx:generated:agents-md -->'), 1);
      assert.equal(countOccurrences(agentsContent, NOMX_MANAGED_AGENTS_START_MARKER), 0);
      assert.equal(countOccurrences(agentsContent, NOMX_MANAGED_AGENTS_END_MARKER), 0);
      assert.match(
        agentsContent,
        new RegExp(`\\| Frontier \\(leader\\) \\| \`${expectedContext.frontierModel}\` \\| high \\|`),
      );
  ...
  it('refreshes only the explicit NOMX-owned model block inside a user-authored AGENTS.md', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nomx-setup-agents-'));
    const restoreTty = setMockTty(false);
    const home = join(wd, 'home');
    const restoreHome = setMockHome(home);
    try {
      ...
      assert.doesNotMatch(agentsContent, /legacy-frontier/);
      assert.doesNotMatch(agentsContent, /<!-- nomx:generated:agents-md -->/);
    } finally {
      restoreHome();
      restoreTty();
      await rm(wd, { recursive: true, force: true });
  ...
  it('preserves a title-only user-authored AGENTS.md by default when no NOMX markers exist', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nomx-setup-agents-'));
    const restoreTty = setMockTty(false);
    const home = join(wd, 'home');
    const restoreHome = setMockHome(home);
    ...
    try {
      ...
      assert.match(output, /WARNING: Existing AGENTS\.md .* lacks NOMX contract markers/);
  ...
  it('preserves symlinked user-scope AGENTS.md during plugin-mode cleanup', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nomx-setup-plugin-agents-symlink-'));
    const restoreTty = setMockTty(false);
    const home = join(wd, 'home');
    const restoreHome = setMockHome(home);
    ...
    try {
      ...
      await writeFile(
        dotfilesAgentsPath,
        addGeneratedAgentsMarker('# nomx - Intelligent Multi-Agent Orchestration\n\nDotfiles-owned guidance.\n'),
      );
  ...
  it('merges plugin-mode NOMX-managed sections into an unmarked user AGENTS.md when explicitly requested', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nomx-setup-plugin-agents-merge-'));
    const restoreTty = setMockTty(false);
    const home = join(wd, 'home');
    const restoreHome = setMockHome(home);
    const codexAgentsPath = join(home, '.codex', 'AGENTS.md');
    ...
    try {
      ...
      assert.match(output, /Merged plugin-mode NOMX-managed AGENTS\.md sections into/);
      assert.match(output, /agents_md: updated=1, unchanged=0, backed_up=1, skipped=0, removed=0/);
      assert.match(agentsContent, /^# Personal Instructions/);
      assert.match(agentsContent, /Keep this custom user guidance\./);
      assert.match(agentsContent, new RegExp(NOMX_MANAGED_AGENTS_START_MARKER));
      assert.match(agentsContent, new RegExp(NOMX_MANAGED_AGENTS_END_MARKER));
      assert.match(agentsContent, /<!-- nomx:generated:agents-md -->/);
      assert.match(agentsContent, /# nomx - Intelligent Multi-Agent Orchestration/);
      assert.equal(existsSync(join(home, '.nomx', 'backups', 'setup')), true);
    } finally {
      restoreHome();
      restoreTty();
      await rm(wd, { recursive: true, force: true });
  ...
  it('keeps plugin-mode explicit AGENTS.md merge idempotent on repeated runs', async () => {
    ...
    try {
      ...
      assert.equal(countOccurrences(secondContent, NOMX_MANAGED_AGENTS_START_MARKER), 1);
      assert.equal(countOccurrences(secondContent, NOMX_MANAGED_AGENTS_END_MARKER), 1);
      assert.equal(countOccurrences(secondContent, '# nomx - Intelligent Multi-Agent Orchestration'), 1);
    } finally {
      restoreHome();
      restoreTty();
      await rm(wd, { recursive: true, force: true });
  ...
  it('does not ask for plugin AGENTS defaults during explicit plugin-mode AGENTS.md merge', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nomx-setup-plugin-agents-merge-prompt-'));
    const restoreTty = setMockTty(true);
    const home = join(wd, 'home');
    const restoreHome = setMockHome(home);
    const codexAgentsPath = join(home, '.codex', 'AGENTS.md');
  ...
  it('skips plugin-mode explicit AGENTS.md merge for symlinked user AGENTS.md', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nomx-setup-plugin-agents-symlink-merge-'));
    const restoreTty = setMockTty(false);
    const home = join(wd, 'home');
    const restoreHome = setMockHome(home);
    const dotfilesAgentsPath = join(wd, 'dotfiles', '.codex', 'AGENTS.md');
  ...
  it('persists explicit false and clear policies when a plugin symlink skips the user AGENTS.md write', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nomx-setup-plugin-agents-symlink-policy-'));
    const restoreTty = setMockTty(false);
    const home = join(wd, 'home');
    const restoreHome = setMockHome(home);
    const dotfilesAgentsPath = join(wd, 'dotfiles', '.codex', 'AGENTS.md');
    ...
    try {
      ...
      await writeFile(dotfilesAgentsPath, dotfilesContent);
      await symlink(dotfilesAgentsPath, codexAgentsPath);
      await mkdir(join(home, '.nomx'), { recursive: true });
      const statePath = join(home, '.nomx', 'setup-scope.json');
  ...
  it('skips plugin-mode explicit AGENTS.md merge for broken symlinked user AGENTS.md', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nomx-setup-plugin-agents-broken-symlink-merge-'));
    const restoreTty = setMockTty(false);
    const home = join(wd, 'home');
    const restoreHome = setMockHome(home);
  ...
  it('skips plugin-mode AGENTS defaults for symlinked user AGENTS.md even when prompted overwrite would be accepted', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nomx-setup-plugin-agents-symlink-default-'));
    const restoreTty = setMockTty(true);
    const home = join(wd, 'home');
    const restoreHome = setMockHome(home);
    const dotfilesAgentsPath = join(wd, 'dotfiles', '.codex', 'AGENTS.md');
  ...
  it('skips plugin-mode AGENTS defaults for broken symlinked user AGENTS.md', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nomx-setup-plugin-agents-broken-symlink-default-'));
    const restoreTty = setMockTty(false);
    const home = join(wd, 'home');
    const restoreHome = setMockHome(home);
    const missingAgentsPath = join(wd, 'dotfiles', '.codex', 'AGENTS.md');
  ...
  it('preserves user-owned NOMX policy blocks during forced plugin defaults refresh', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nomx-setup-plugin-agents-policy-'));
    const restoreTty = setMockTty(false);
    const home = join(wd, 'home');
    const restoreHome = setMockHome(home);
    const codexAgentsPath = join(home, '.codex', 'AGENTS.md');
    ...
    try {
      ...
      assert.match(agents, /# nomx - Intelligent Multi-Agent Orchestration/);
      assert.match(agents, /Durable local operator rule: never drop this\./);
      assert.equal(countOccurrences(agents, NOMX_USER_POLICY_START_MARKER), 1);
      assert.equal(countOccurrences(agents, NOMX_USER_POLICY_END_MARKER), 1);
    } finally {
      restoreHome();
      restoreTty();
      await rm(wd, { recursive: true, force: true });
  ...
  it('merges NOMX-managed sections into an unmarked user-authored AGENTS.md when explicitly requested', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nomx-setup-agents-'));
    const restoreTty = setMockTty(false);
    const home = join(wd, 'home');
    const restoreHome = setMockHome(home);
    ...
    try {
      ...
      assert.match(agentsContent, new RegExp(NOMX_MANAGED_AGENTS_END_MARKER));
      assert.match(agentsContent, /# nomx - Intelligent Multi-Agent Orchestration/);
      assert.equal(existsSync(join(wd, '.nomx', 'backups', 'setup')), true);
    } finally {
      restoreHome();
      restoreTty();
      await rm(wd, { recursive: true, force: true });
  ...
  it('refreshes the managed model table inside an explicit merged AGENTS.md block', async () => {
    ...
    try {
      ...
      assert.doesNotMatch(agentsContent, /legacy-frontier/);
      assert.doesNotMatch(agentsContent, /legacy-spark/);
      assert.equal(countOccurrences(agentsContent, NOMX_MANAGED_AGENTS_START_MARKER), 1);
    } finally {
      restoreHome();
      restoreTty();
      await rm(wd, { recursive: true, force: true });
  ...
  it('skips explicit AGENTS.md merge during an active project session', async () => {
    ...
    try {
      ...
      assert.equal(existsSync(join(wd, '.nomx', 'backups', 'setup')), false);
      assert.deepEqual(
        JSON.parse(await readFile(join(wd, '.nomx', 'setup-scope.json'), 'utf-8')),
  ...
  it('persists explicit false and clear policies when an active session skips the project AGENTS.md write', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nomx-setup-agents-active-policy-'));
    const restoreTty = setMockTty(false);
    const home = join(wd, 'home');
    const restoreHome = setMockHome(home);
    const existing = '# active session file\n';
  ...
  it('skips plugin-mode explicit AGENTS.md merge during an active project session', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nomx-setup-plugin-agents-active-'));
    const restoreTty = setMockTty(false);
    const home = join(wd, 'home');
    const restoreHome = setMockHome(home);
  ...
  it('skips plugin-mode AGENTS defaults during an active project session', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nomx-setup-plugin-agents-active-default-'));
    const restoreTty = setMockTty(true);
    const home = join(wd, 'home');
    const restoreHome = setMockHome(home);
    const existing = '# active plugin project file\n';
