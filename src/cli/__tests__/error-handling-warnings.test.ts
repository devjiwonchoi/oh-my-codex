...
describe('error-handling warning guards', () => {
  it('uses warning logs for watcher lifecycle best-effort failures', async () => {
    ...
    assert.ok(!source.includes("await mkdir(join(cwd, '.nomx', 'state'), { recursive: true }).catch(() => {});"));
  ...
  it('hides Windows child windows for retained prompt and notification helpers', async () => {
