...
const AGENTS_USAGE = [
  'Usage:',
  '  nomx agents list [--scope user|project]',
  '  nomx agents add <name> [--scope user|project] [--force]',
  '  nomx agents edit <name> [--scope user|project]',
  '  nomx agents remove <name> [--scope user|project] [--force]',
  '',
...
function inferMutationScope(cwd = process.cwd()): AgentScope {
  const persistedScopePath = join(cwd, '.nomx', 'setup-scope.json');
...
export async function agentsCommand(args: string[]): Promise<void> {
  ...
  switch (subcommand) {
    ...
    case 'add': {
      const name = args[1];
      assert.ok(name, 'Usage: nomx agents add <name>');
    ...
    case 'edit': {
      const name = args[1];
      assert.ok(name, 'Usage: nomx agents edit <name>');
    ...
    case 'remove': {
      const name = args[1];
      assert.ok(name, 'Usage: nomx agents remove <name>');
