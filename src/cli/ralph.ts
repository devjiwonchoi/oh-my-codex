...
/** @deprecated Retained as a no-op compatibility export for callers upgrading from deslop-enabled Ralph. */
export function buildRalphChangedFilesSeedContents(): string {
  return '# Ralph changed files tracking is no longer required';
}
...
export function buildRalphAppendInstructions(
  ...
  return [
    ...
    '- When it does not (`role_routing_unavailable`, for example a Codex App `spawn_agent` surface exposing only `task_name`, `message`, and `fork_turns`), do not fabricate `agent_type`; follow the NOMX adapted role-pass protocol: use `CODEX_THREAD_ID` as the authenticated current leader identity, record the pre-validated role intent in the NOMX subagent ledger with `nomx ralplan role-intent write --role <role> --parent-thread "$CODEX_THREAD_ID" --json`, use the receipt `spawn_task_name` exactly, and never fake the role via a prompt label.',
    '- On a `role_routing_unavailable` surface, preserve legacy Ralph tier intent only as task-scoping guidance (LOW for narrow lookup, STANDARD for bounded implementation, THOROUGH for complex or high-risk analysis). Use only `task_name`, `message`, and optional `fork_turns`; inherit session defaults and do not invent `model`, `reasoning_effort`, `run_in_background`, or other dispatch fields.',
    ...
    '- Rerun the required tests/build/lint after final fixes and read the output before completion.',
    '</ralph_native_subagents>',
  ].join('\n');
}
...
async function writeRalphSessionFiles(
  ...
  await writeFile(
    instructionsPath,
    `${buildRalphAppendInstructions(task, { approvedHint: options.approvedHint ?? null })}\n`,
  );
...
export async function ralphCommand(args: string[]): Promise<void> {
  ...
  const sessionFiles = await writeRalphSessionFiles(cwd, task, { approvedHint });
