...
function buildLaunchHints(
  ...
  if (mode === 'team') {
    return {
      shellCommand: `codex ${toQuotedCliArg(task)}`,
      skillCommand: `Use ${recommendedHeadcount} native Codex subagents led by ${fallbackRole}`,
      rationale: 'Use bounded native Codex subagents for parallel delivery and verification within the current task.',
    };
  }

  return {
    shellCommand: `nomx ralph ${toQuotedCliArg(task)}`,
...
function buildVerificationPlan(
  ...
  if (mode === 'team') {
    ...
    return {
      summary: 'Use native Codex subagents for coordinated execution and verification: delivery lanes run in parallel while the leader integrates fresh evidence.',
      checkpoints: [
        'Spawn bounded native Codex subagents with explicit scopes and verifiable outputs.',
        `Keep ${qualityLane?.role ?? 'the verification lane'} focused on tests, regression coverage, and evidence capture before leader sign-off.`,
